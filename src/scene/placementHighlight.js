...
// Создаёт шейдерный материал с белой рамкой и редкими волнами, уходящими вверх.
  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      float wavePulse(float offset) {
        ...
        float tail = smoothstep(0.42, 0.18, pos);
        return front * tail;
      }

      void main() {
        float baseGlow = smoothstep(1.0, 0.82, vUv.y);
        float wave = wavePulse(0.0);
        float fadeTop = smoothstep(1.0, 0.48, vUv.y);
        float lateral = smoothstep(0.0, 0.018, vUv.x) * smoothstep(0.0, 0.018, 1.0 - vUv.x);
        // Делаем подсветку менее навязчивой за счёт уменьшения общей интенсивности.
        float intensity = (baseGlow * 0.45 + wave * 1.1) * fadeTop * lateral * 0.6;

  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        float intensity = (baseGlow * 0.6 + wave * 1.5) * fadeTop * lateral;
        float alpha = clamp(intensity, 0.0, 1.0) * 0.8;
...
function createFrameGroup(tile, material) {
  ...
  // Высота рамки уменьшена в пять раз, чтобы сделать эффект компактнее.
  const frameHeight = Math.max(width, depth) * 0.65 * 0.2;

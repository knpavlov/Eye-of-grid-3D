...
function createOverlayMaterial(THREE) {
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        float baseGlow = smoothstep(1.0, 0.84, vUv.y);
        float wave = wavePulse(0.0);
        float fadeTop = smoothstep(1.0, 0.52, vUv.y);
        float lateral = smoothstep(0.0, 0.02, vUv.x) * smoothstep(0.0, 0.02, 1.0 - vUv.x);
        // Усиливаем прозрачность и уменьшаем вклад волн для более мягкого эффекта.
        float intensity = (baseGlow * 0.45 + wave * 1.05) * fadeTop * lateral * 0.65;

...
export async function animateDrawnCardToHand(cardTpl) {
  ...
  // Начальное проявление теперь совпадает с положением и углом карты в руке
  ...
  await new Promise(resolve => {
    ...
    const flyDuration = 0.46; // Полёт в руку быстрее примерно на 50%
    // Одновременно проявляем карту и отправляем её в руку
    tl.to(allMaterials, { opacity: 1, duration: 0.8, ease: 'power2.out' })
      .to(big.position, { x: target.position.x, y: target.position.y, z: target.position.z, duration: flyDuration, ease: 'power2.inOut' }, '<')
      .to(big.rotation, { x: target.rotation.x, y: target.rotation.y, z: target.rotation.z, duration: flyDuration, ease: 'power2.inOut' }, '<')
      .to(big.scale, { x: target.scale.x, y: target.scale.y, z: target.scale.z, duration: flyDuration, ease: 'power2.inOut' }, '<');
    try {

...
// Планирует траекторию полёта карты, чтобы она подлетала выше и сбрасывала высоту в конце
function computeFlightPositionPlan(startPosition, targetPosition, flightDuration, tune = {}) {
  const THREE = getTHREE();
  const total = Math.max(0, typeof flightDuration === 'number' ? flightDuration : 0);
  const settleShareRaw = Number.isFinite(tune.positionSettleShare) ? tune.positionSettleShare : DEFAULT_POSITION_SETTLE_SHARE;
  const settleShare = clamp01(settleShareRaw);
  const settleDuration = total * settleShare;
  const travelDuration = Math.max(0, total - settleDuration);

  const start = (startPosition instanceof THREE.Vector3)
    ? startPosition.clone()
    : new THREE.Vector3(startPosition?.x || 0, startPosition?.y || 0, startPosition?.z || 0);
  const target = (targetPosition instanceof THREE.Vector3)
    ? targetPosition.clone()
    : new THREE.Vector3(targetPosition?.x || 0, targetPosition?.y || 0, targetPosition?.z || 0);

  const arcProgressRaw = Number.isFinite(tune.flightArcProgress) ? tune.flightArcProgress : DEFAULT_FLIGHT_ARC_PROGRESS;
  const arcProgress = clamp01(arcProgressRaw);
  const arcLift = Number.isFinite(tune.flightArcLift) ? tune.flightArcLift : DEFAULT_FLIGHT_ARC_LIFT;
  const arcForward = Number.isFinite(tune.flightArcForward) ? tune.flightArcForward : DEFAULT_FLIGHT_ARC_FORWARD;
  const arcSide = Number.isFinite(tune.flightArcSideOffset) ? tune.flightArcSideOffset : DEFAULT_FLIGHT_ARC_SIDE_OFFSET;

  const midPosition = start.clone().lerp(target, arcProgress);
  midPosition.x += arcSide;
  midPosition.y += arcLift;
  midPosition.z += arcForward;
...
// Параметры траектории полёта, чтобы карта подлетала сверху и опускалась мягче
const DEFAULT_POSITION_SETTLE_SHARE = 0.28;
const DEFAULT_FLIGHT_ARC_LIFT = 1.05;
const DEFAULT_FLIGHT_ARC_FORWARD = -0.75;
const DEFAULT_FLIGHT_ARC_SIDE_OFFSET = 0;
const DEFAULT_FLIGHT_ARC_PROGRESS = 0.62;
const DRAW_POSITION_SETTLE_EASE = 'sine.in';
...
export async function animateDrawnCardToHand(cardTpl) {
  ...
  const flightStartPosition = big.position.clone();
  ...
  const positionPlan = computeFlightPositionPlan(flightStartPosition, target.position, flightDuration, T);
  const {
    travelDuration: positionTravelDuration,
    settleDuration: positionSettleDuration,
    midPosition
  } = positionPlan;
  ...
  const {
    ...
    rotationLeanFraction,
    tiltDuration,
    settleDuration: rotationSettleDuration
  } = rotationPlan;
  ...
  try {
    await new Promise(resolve => {
      ...
      if (positionTravelDuration > 0.0001 && midPosition) {
        tl.to(big.position, {
          x: midPosition.x,
          y: midPosition.y,
          z: midPosition.z,
          duration: positionTravelDuration,
          ease: DRAW_FLIGHT_EASE
        }, 'flightMotion');
      }

      const settleStartLabel = `flightMotion+=${Math.max(positionTravelDuration, 0)}`;
      if (positionSettleDuration > 0.0001) {
        tl.to(big.position, {
          x: target.position.x,
          y: target.position.y,
          z: target.position.z,
          duration: positionSettleDuration,
          ease: DRAW_POSITION_SETTLE_EASE
        }, settleStartLabel);
      } else {
        tl.set(big.position, {
          x: target.position.x,
      ...
      if (rotationSettleDuration > 0.0001 && eulersDiffer(flightRotation, arrivalRotation)) {
        tl.to(big.rotation, {
          x: arrivalRotation.x,
          y: arrivalRotation.y,
          z: arrivalRotation.z,
          duration: rotationSettleDuration,

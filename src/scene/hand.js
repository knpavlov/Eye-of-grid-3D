...
// Строит массив трансформов для всей руки, чтобы переиспользовать расчёты
function computeHandLayout(total) {
  const layout = [];
  const cappedTotal = Math.max(0, Math.floor(total || 0));
  for (let i = 0; i < cappedTotal; i += 1) {
    layout.push(computeHandTransform(i, cappedTotal));
  }
  return layout;
}
...
// Плавно переводит значение в диапазон [0, 1], смягчая переходы
...
// Насколько долго карта сохраняет исходный наклон перед началом плавного поворота
const DEFAULT_ROTATION_LEAN_DELAY = 0.06;
// Какую долю «поворота» выполняем до финальной досадки
const DEFAULT_DRAW_ROTATION_LEAN_FRACTION = 0.6;
// Какая часть финального промежутка отведена на удержание полётного наклона

...
// Строит полный набор трансформов для руки, чтобы переиспользовать их при анимациях
function computeHandLayout(total) {
  const count = Math.max(0, total | 0);
  const transforms = [];
  for (let i = 0; i < count; i += 1) {
    transforms.push(computeHandTransform(i, count));
  }
  return transforms;
}
...
// Плавно нормализует значение в диапазоне [0, 1], убирая резкие границы
...
function relayoutHandDuringDraw(handMeshes, layoutAfterDraw, duration) {
  ...
  const targetLayout = Array.isArray(layoutAfterDraw) ? layoutAfterDraw : [];

  handMeshes.forEach((mesh, idx) => {
    const fallbackTotal = targetLayout.length > 0 ? targetLayout.length : handMeshes.length;
    const t = targetLayout[idx] || computeHandTransform(idx, fallbackTotal);

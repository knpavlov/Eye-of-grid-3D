...
// Смещение промежуточной точки полёта, чтобы карта подлетала чуть выше и снаружи веера
const DEFAULT_DRAW_APPROACH_RISE = 0.7;
const DEFAULT_DRAW_APPROACH_LATERAL = 0.55;
const DEFAULT_DRAW_APPROACH_DEPTH = 0.0;
...
export async function animateDrawnCardToHand(cardTpl) {
  ...
  const approachRise = Number.isFinite(T.approachRise) ? T.approachRise : DEFAULT_DRAW_APPROACH_RISE;
  const approachLateral = Number.isFinite(T.approachLateral)
    ? T.approachLateral
    : DEFAULT_DRAW_APPROACH_LATERAL;
  const approachDepth = Number.isFinite(T.approachDepth) ? T.approachDepth : DEFAULT_DRAW_APPROACH_DEPTH;
  const approachPosition = target.position.clone();
  if (approachRise !== 0) approachPosition.y += approachRise;
  if (approachDepth !== 0) approachPosition.z += approachDepth;
  if (approachLateral !== 0) {
    const lateralSign = target.position.x >= 0 ? 1 : -1;
    approachPosition.x += approachLateral * lateralSign;
  }
  try {
  ...
  try {
    await new Promise(resolve => {
      ...
      const positionUsesTwoStage =
        rotationLead > 0.0001 && settleStartTime > 0.0001 &&
        (approachRise !== 0 || approachDepth !== 0 || (approachLateral !== 0 && Math.abs(target.position.x) > 0.0001));

      if (positionUsesTwoStage) {
        tl.to(big.position, {
          x: approachPosition.x,
          y: approachPosition.y,
          z: approachPosition.z,
          duration: settleStartTime,

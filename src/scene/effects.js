...
export function magicBurst(pos, durationSec = 0.5) {
  const THREE = window.THREE; const gsap = window.gsap;
  const effectsGroup = window.effectsGroup || window.__scene?.getCtx()?.effectsGroup;
  ...
  beam.position.y = 1.0;
  group.add(beam);
  gsap.to(beam.scale, { x: 1.5, z: 1.5, duration: durationSec / 2, yoyo: true, repeat: 1, ease: 'power2.out' });
  ...
  for (let i = 0; i < count; i++) {
    ...
    positions[i * 3 + 1] = 0;
    positions[i * 3 + 2] = pz;
    velocities.push(new THREE.Vector3(px * 4, Math.random() * 4 + 2, pz * 4));
  }

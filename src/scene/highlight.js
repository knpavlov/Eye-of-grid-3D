...
async function loadPP() {
  if (!PP) {
    const urls = [
      // Сначала пробуем локальную копию (можно положить в src/lib)
      '../lib/postprocessing.esm.js',
      // Путь к последней версии major=6 на jsDelivr
      'https://cdn.jsdelivr.net/npm/postprocessing@6/build/postprocessing.esm.js',
      // Запасной CDN на случай недоступности первого
      'https://unpkg.com/postprocessing@6/build/postprocessing.esm.js',
    ];
    for (const url of urls) {
      try {
        ...
        console.warn('Не удалось загрузить postprocessing из', url, e);
      }
    }
  }
  return PP;
...
export function highlightTiles(cells = []) {
  (async () => {
    ...
    const lib = await loadPP();
    if (!lib) { console.warn('postprocessing не загружен, подсветка недоступна'); return; }
    const { EffectComposer, RenderPass, EffectPass, GodRaysEffect, BlendFunction } = lib;
    ...
    // Для каждой точки подсветки создаём отдельный GodRaysEffect
    g.children.forEach((src, idx) => {
      const effect = new GodRaysEffect(camera, src, {
      ...
      // Последний пасс выводим на экран
      if (idx === g.children.length - 1) pass.renderToScreen = true;

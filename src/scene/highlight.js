...
async function loadPP() {
  if (!PP) {
    const urls = [
      // Локальная копия (если присутствует в проекте)
      '../vendor/postprocessing.esm.js',
      // Быстрая ESM‑обёртка
      'https://esm.sh/postprocessing@6.35.1?bundle',
      // Официальные CDN
      'https://cdn.jsdelivr.net/npm/postprocessing@6.35.1/dist/postprocessing.esm.js',
      'https://unpkg.com/postprocessing@6.35.1/dist/postprocessing.esm.js',
    ];
    for (const url of urls) {
      try {
        PP = await import(url);
        console.info('postprocessing загружен из', url);
        break;
      } catch (e) {

...
// Ленивый импорт библиотеки postprocessing с обработкой ошибок
let PP = null;
async function loadPP() {
  if (!PP) {
    try {
      // Используем стабильный CDN unpkg, т.к. jsDelivr выдавал 404
      PP = await import('https://unpkg.com/postprocessing@6.35.1/build/postprocessing.esm.js');
    } catch (err) {
      console.error('[highlight] Не удалось загрузить postprocessing', err);
      PP = null;
    }
  }
  return PP;
...
export function highlightTiles(cells = []) {
  (async () => {
    ...
    if (!pp) return; // Если модуль не загрузился, выходим тихо
    const { EffectComposer, RenderPass, EffectPass, GodRaysEffect, BlendFunction } = pp;

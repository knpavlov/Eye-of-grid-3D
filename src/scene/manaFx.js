...
export function buildManaGainPlan({
  ...
  try {
    ...
    const reserve = () => {
      if (reserveCalled) return reserveSucceeded;
      reserveCalled = true;
      try {
        const reserveFn = (typeof window !== 'undefined')
          ? (window.__ui?.mana?.reserveIncomingMana)
          : null;
        if (typeof reserveFn !== 'function') {
          reserveSucceeded = false;
          return reserveSucceeded;
        }
        const byOwner = new Map();
        for (const ev of events) {
          if (!Number.isFinite(ev.owner)) continue;
          byOwner.set(ev.owner, (byOwner.get(ev.owner) || 0) + ev.amount);
        }
        for (const [owner, amount] of byOwner.entries()) {
          if (amount > 0) reserveFn(owner, amount);
        }
        reserveSucceeded = byOwner.size > 0;
      } catch (err) {
        console.error('[manaFx] Не удалось зарезервировать слоты маны:', err);
        reserveSucceeded = false;
      }
      return reserveSucceeded;
    };
    ...
    return { events, schedule, getOrigin, reserve };
  } catch (err) {

  ...
  try {
    ...
    const blockApi = (() => {
      try {
        ...
        const uiMana = window.__ui?.mana || {};
        const reserve = uiMana.reserveManaBlocks || window.reserveManaBlocks || null;
        const release = uiMana.releaseManaBlocks || window.releaseManaBlocks || null;
        if (typeof reserve === 'function' && typeof release === 'function') {
          return { reserve, release };
        }
      } catch (err) {
        console.error('[manaFx] Не удалось получить API блокировки маны:', err);
      }
      return null;
    })();
    let externalBlocksEnabled = !!blockApi;
    ...
    const registerPending = (owner, amount) => {
      if (!Number.isFinite(owner) || amount <= 0) return;
      const prev = pendingByOwner.get(owner) || 0;
      pendingByOwner.set(owner, prev + amount);
    };

    const resolveWatcherOrigin = (watcher) => {
      try {
        if (!watcher || watcher.r == null || watcher.c == null) return null;
        const row = watcher.r;
        const col = watcher.c;
    ...
    const addEvent = (owner, origin, amount, slotStart, startDelay) => {
      ...
      registerPending(owner, normalizedAmount);
    };
    ...
    for (const entry of (Array.isArray(manaGainEntries) ? manaGainEntries : [])) {
      ...
      let origin = null;
      if (entry.type === 'ALLY_DEATH' && entry.watcher) {
        origin = resolveWatcherOrigin(entry.watcher);
      }
      if (!origin) {
        const key = makeKey(death);
    ...
    const releaseOne = (owner) => {
      if (!externalBlocksEnabled || !blockApi) return;
      try {
        blockApi.release(owner, 1);
      } catch (err) {
        console.error('[manaFx] Не удалось освободить блок маны:', err);
      }
    };

    const reserveBlocks = () => {
      if (!externalBlocksEnabled || reserveApplied) return false;
      if (!blockApi || pendingByOwner.size === 0) return false;
      try {
        for (const [owner, total] of pendingByOwner.entries()) {
          blockApi.reserve(owner, total);
        }
        reserveApplied = true;
        return true;
      } catch (err) {
        console.error('[manaFx] Не удалось зарезервировать блоки маны:', err);
        externalBlocksEnabled = false;
        return false;
      }
    };
    ...
    const schedule = () => {
      ...
      reserveBlocks();
      const useExternal = externalBlocksEnabled && reserveApplied;
    ...
    return { events, schedule, getOrigin, reserve: reserveBlocks };
  } catch (err) {
    console.error('[manaFx] Ошибка при подготовке плана маны:', err);
    return { events: [], schedule: () => {}, getOrigin: () => null, reserve: () => false };
  }
}

...
export function buildManaGainPlan({
  ...
  try {
    ...
    const affectedOwners = new Set(events.map(ev => ev.owner));
        ...
        try {
          ...
          if (g?.players) {
            for (const owner of affectedOwners) {
              ...
              const beforeMana = initialMana.get(owner);
              if (beforeMana == null) continue;
              const player = g.players[owner];
              if (!player) continue;
              if (player._beforeMana == null) {
                player._beforeMana = beforeMana;
              }
            }
            if (affectedOwners.size && typeof window.updateUI === 'function') {
              window.updateUI();
            }
          }
        } catch (err) {
          console.warn('[manaFx] Не удалось проставить _beforeMana перед анимацией прироста:', err);
        }
      }

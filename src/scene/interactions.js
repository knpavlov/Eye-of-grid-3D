...
export function placeUnitWithDirection(direction) {
  ...
  if (!alive) {
    ...
    if (gameState.players?.[owner]) {
      gameState.players[owner].mana = capMana((gameState.players[owner].mana || 0) + 1);
    }
    gameState.board[row][col].unit = null;
    const manaGain = applyManaGainOnDeaths(gameState, deathInfo, { boardState: gameState });
    const ctx = getCtx();
    const THREE = ctx.THREE || (typeof window !== 'undefined' ? window.THREE : undefined);
    const manaPlan = buildManaGainPlan({
      playersBefore,
      deaths: deathInfo,
      manaGainEntries: manaGain.entries || [],

...
export function placeUnitWithDirection(direction) {
  ...
  if (!alive) {
    ...
    const manaFromDeath = applyManaGainOnDeaths(gameState, deathInfo, { boardState: gameState });
    if (Array.isArray(manaFromDeath?.logs) && manaFromDeath.logs.length) {
      for (const text of manaFromDeath.logs) {
    ...
    const manaPlan = buildManaGainPlan({
      playersBefore,
      deaths: deathInfo,
      manaGainEntries: Array.isArray(manaFromDeath?.entries) ? manaFromDeath.entries : [],
      tileMeshes: ctx.tileMeshes,
      THREE,
    });
    try { manaPlan?.schedule?.(); } catch (err) {
      console.warn('[summon] Не удалось запустить анимацию маны после гибели при призыве:', err);
    }

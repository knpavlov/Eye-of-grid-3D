...
const CARD_FACE_LAYOUT = {
  ...
  nameY: 232,
  typeY: 288,
  art: { x: 48, y: 344, width: 736, height: 432 },
  textGap: 36,
  textBottomGap: 22,
  diagramsBottom: 1158,
  diagramLabelGap: 32,
...
export function drawCardFace(ctx, cardData, width, height, hpOverride = null, atkOverride = null, opts = {}) {
  ...
  if (img && img.complete && !(typeof location !== 'undefined' && location.protocol === 'file:')) {
    ...
    ctx.font = `500 ${Math.max(ps(32), 12)}px "Noto Sans", "Helvetica", sans-serif`;
  ...
  const descFontPx = Math.max(ps(26), 12);
  ...
  if (cardData.type === 'UNIT') {
    diagramCell = Math.max(Math.round(ps(26)), 9);
    diagramGap = Math.max(Math.round(ps(5)), 2);
  ...
  if (cardData.type === 'UNIT') {
    ...
    const cell = diagramCell ?? Math.max(Math.round(ps(26)), 9);
    const gap = diagramGap ?? Math.max(Math.round(ps(5)), 2);
    const gridW = cell * 3 + gap * 2;
    const spacing = Math.max(Math.round(ps(48)), 14);
    ...
    schemes.forEach((scheme, idx) => {
      ...
      if (labelRaw) {
        ...
        ctx.font = `600 ${Math.max(ps(26), 11)}px "Noto Sans", "Helvetica", sans-serif`;

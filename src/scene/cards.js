...
const CARD_FACE_LAYOUT = {
  ...
  textGap: 36,
  textPadding: { top: 50, horizontal: 50 },
  textBottomGap: 22,
  diagramsBottom: 1148,
  diagramLabelGap: 26,
  statsBaseline: 1158
...
export function drawCardFace(ctx, cardData, width, height, hpOverride = null, atkOverride = null, opts = {}) {
  ...
  const textPadding = layout.textPadding || { top: 0, horizontal: 0 };
  const textBaseX = layout.art.x + (textPadding.horizontal || 0);
  const textBaseWidth = Math.max(0, layout.art.width - 2 * (textPadding.horizontal || 0));
  const textX = px(textBaseX);
  const textY = py(layout.art.y + layout.art.height + layout.textGap + (textPadding.top || 0));
  const textWidth = px(textBaseWidth);

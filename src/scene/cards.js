...
const CARD_FACE_LAYOUT = {
  ...
  nameY: 168,
  typeY: 224,
  art: { x: 48, y: 242, width: 736, height: 432 },
  textPaddingX: 50,
  textPaddingTop: 50,
  textGap: 36,
  textBottomGap: 22,
...
// Получаем шаблон карты из глобальной базы, если на вход передана только строка
function resolveCardData(card) {
  if (!card) return null;
  if (typeof card === 'string') {
    try {
      if (typeof window !== 'undefined' && window.CARDS) {
        return window.CARDS[card] || null;
      }
    } catch {}
    return { id: card };
  }
  if (card && typeof card === 'object' && card.id && (!card.name || card.name === card.id)) {
    try {
      if (typeof window !== 'undefined' && window.CARDS && window.CARDS[card.id]) {
        return { ...window.CARDS[card.id], ...card };
      }
    } catch {}
  }
  return card;
...
// Формируем список ключей для поиска и кеширования иллюстраций
function getIllustrationKeys(cardData) {
  const keys = [];
  if (!cardData) return keys;
  if (cardData.id) {
    keys.push(String(cardData.id));
    keys.push(String(cardData.id).toLowerCase());
  }
  if (cardData.name) {
    const slug = String(cardData.name)
      .toLowerCase()
      .replace(/[^a-z0-9\s_-]/g, '')
      .replace(/\s+/g, '_');
    if (slug) {
      keys.push(slug);
      keys.push(slug.replace(/_/g, '-'));
    }
  }
  return keys.filter(Boolean);
}

// Создаём список кандидатов для загрузки иллюстраций
function getIllustrationCandidates(cardData) {
  const keys = getIllustrationKeys(cardData);
  return keys.map(key => `card images/${key}.png`);
}

// Сохраняем успешно загруженную иллюстрацию под всеми подходящими ключами
function registerIllustration(cardData, image) {
  const keys = getIllustrationKeys(cardData);
  keys.forEach(key => {
    if (!key) return;
...
// Ищем подходящую иллюстрацию в кеше
function lookupIllustration(cardData) {
  const keys = getIllustrationKeys(cardData);
  for (const key of keys) {
    if (key && CARD_IMAGES[key]) return CARD_IMAGES[key];
  }
  return null;
}

// Запускаем загрузку иллюстрации, если она ещё не кеширована
export function ensureCardIllustration(card) {
  const cardData = resolveCardData(card);
  if (!cardData) return;
  if (lookupIllustration(cardData)) return;
  const candidates = getIllustrationCandidates(cardData);
  if (!candidates.length) return;
  const pendingKey = getIllustrationKeys(cardData)[0];
  ...
  (function tryLoad(i) {
    ...
    const im = new Image();
    im.onload = () => {
      registerIllustration(cardData, im);
      if (pendingKey) CARD_PENDING[pendingKey] = false;
...
// Массовая предзагрузка иллюстраций для списков карт
export function preloadCardIllustrations(cards = []) {
  const list = Array.isArray(cards) ? cards : [cards];
  list.forEach(card => {
    try { ensureCardIllustration(card); } catch {}
  });
}
...
export function drawCardFace(ctx, cardData, width, height, hpOverride = null, atkOverride = null, opts = {}) {
  ...
  let nameFont = Math.max(ps(34), 14);
  const minNameFont = Math.max(ps(25), 12);
  const nameMaxWidth = width - px(208);
  while (true) {
  ...
  const nameShadowBlur = Math.max(Math.round(nameFont * 0.6), 10);
  ctx.shadowBlur = nameShadowBlur;
  ctx.shadowOffsetY = Math.max(Math.round(nameFont * 0.1), 2);
  ...
  const limitLabel = formatLimitLabel(cardData.cardLimit);
  if (limitLabel) metaParts.push(limitLabel);
  ...
  let img = lookupIllustration(cardData);
  if (!img) {
    ensureCardIllustration(cardData);
    img = lookupIllustration(cardData);
  }
  if (img && img.complete && !(typeof location !== 'undefined' && location.protocol === 'file:')) {
    ...
    ctx.font = `500 ${Math.max(ps(22), 10)}px "Noto Sans", "Helvetica", sans-serif`;
  ...
  const descFontPx = Math.max(ps(13), 10);
  ctx.font = `500 ${descFontPx}px "Noto Sans", "Helvetica", sans-serif`;
  const textPadX = layout.textPaddingX || 0;
  const textPadTop = layout.textPaddingTop || 0;
  const textX = illX + px(textPadX);
  const textY = illY + illH + py(layout.textGap) + py(textPadTop);
  const textWidth = Math.max(0, illW - px(textPadX * 2));
  ...
  const textBottomGap = Math.max(py(layout.textBottomGap), Math.max(ps(12), 10));
  const textMaxY = (cardData.type === 'UNIT')
    ? diagramTop - textBottomGap
    : statsBaseline - textBottomGap;
  wrapText(ctx, rulesText, textX, textY, textWidth, Math.max(Math.round(descFontPx * 1.35), 12), textMaxY);
  ctx.restore();

  // Значения ресурсов
  const drawOrbNumber = (text, position, fontBase, fontFamily = 'Cinzel', weight = '700', minPx = 16) => {
    ctx.save();
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillStyle = '#f8fafc';
    const fontPx = Math.max(ps(fontBase), minPx);
    ctx.font = `${weight} ${fontPx}px "${fontFamily}", "Helvetica", sans-serif`;
    ctx.shadowColor = 'rgba(6, 9, 15, 0.78)';
    ctx.shadowBlur = Math.max(Math.round(fontPx * 0.55), 8);
    ctx.shadowOffsetY = Math.max(Math.round(fontPx * 0.12), 2);
    ctx.fillText(text, px(position.x), py(position.y));
    ctx.restore();
  };

  const costValue = String(cardData.cost ?? 0);
  drawOrbNumber(costValue, layout.cost, 90, 'Cinzel', '700', 14);
  ...
  if (cardData.type === 'UNIT') {
    ...
    drawOrbNumber(String(act), layout.activation, 67, 'Cinzel', '700', 12);
    ...
    const labelGap = Math.max(ps(layout.diagramLabelGap), 12);
    schemes.forEach((scheme, idx) => {
      ...
      if (labelRaw) {
        ...
        ctx.font = `600 ${Math.max(ps(18), 9)}px "Noto Sans", "Helvetica", sans-serif`;
        ctx.fillText(labelRaw, gridX + gridW / 2, gridY + gridHeight + labelGap);
        ctx.restore();
      }
    });

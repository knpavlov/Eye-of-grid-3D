...
const CARD_FACE_LAYOUT = {
  ...
  art: { x: 92, y: 292, width: 648, height: 452 },
  text: {
    x: 124,
    top: 782,
    width: 832 - 248,
...
export function drawCardFace(ctx, cardData, width, height, hpOverride = null, atkOverride = null, opts = {}) {
  ...
  const minNameFont = Math.max(ps(72), 18);
  const nameMaxWidth = width - px(header.maxWidthPadding || 240);
  while (true) {
  ...
  ctx.shadowBlur = Math.max(ps(48), 12);
  ctx.shadowOffsetY = Math.max(ps(6), 2);
  ctx.fillText(displayName, width / 2, py(header.nameY || 236));
  ctx.restore();
  ...
  if (typeLine) {
    ...
    ctx.font = `600 ${Math.max(ps(56), 15)}px "Noto Sans", "Helvetica", sans-serif`;
    ctx.fillText(typeLine, width / 2, py(header.typeY || 296));
    ctx.restore();
  }
  ...
  const illW = px(layout.art.width);
  const illH = py(layout.art.height);
  ctx.save();
  ...
  const textArea = layout.text || {};
  ctx.save();
  ctx.textAlign = 'left';
  ctx.fillStyle = '#d2d7e3';
  const textX = px(textArea.x ?? layout.art.x);
  const textWidth = px(textArea.width ?? layout.art.width);
  const textTop = py(textArea.top ?? (layout.art.y + layout.art.height + 40));
  const textBottomLimit = py(textArea.bottom ?? (layout.statsTop - 120));
  const headingGap = Math.max(py(textArea.headingGap ?? 40), 16);
  const textLineHeight = Math.max(ps(textArea.lineHeight ?? 58), 16);
  const textTitle = (opts && Object.prototype.hasOwnProperty.call(opts, 'rulesTitle'))
    ? opts.rulesTitle
    : 'Description';

  let diagramTop = py(layout.statsTop);
  const diagramsCfg = layout.diagrams || {};
  let diagramCell = null;
  let diagramGap = null;
  let diagramHeight = 0;

  if (cardData.type === 'UNIT') {
    diagramCell = Math.max(Math.round(ps(diagramsCfg.cell ?? 28)), 10);
    diagramGap = Math.max(Math.round(ps(diagramsCfg.gap ?? 6)), 2);
    diagramHeight = diagramCell * 3 + diagramGap * 2;
    diagramTop = py(diagramsCfg.top ?? layout.statsTop - 160);
  }

  const textAreaPadding = Math.max(py(diagramsCfg.bottomPadding ?? 36), 20);
  if (cardData.type === 'UNIT') {
    const maxDiagramTop = py(layout.statsTop) - textAreaPadding - diagramHeight;
    if (diagramTop > maxDiagramTop) diagramTop = maxDiagramTop;
  }

  const textLimitForUnits = cardData.type === 'UNIT'
    ? diagramTop - Math.max(py(diagramsCfg.spacingFromText ?? 32), 18)
    : textBottomLimit;
  const textMaxY = Math.min(textBottomLimit, textLimitForUnits);

  let textCursor = textTop;
  if (rulesText) {
    if (textTitle) {
      ctx.font = `700 ${Math.max(ps(48), 15)}px "Noto Sans", "Helvetica", sans-serif`;
      ctx.fillStyle = 'rgba(226,232,240,0.9)';
      ctx.fillText(textTitle.endsWith('.') ? textTitle : `${textTitle}.`, textX, textCursor);
      textCursor += headingGap;
      ctx.fillStyle = '#d2d7e3';
    }
    ctx.font = `500 ${Math.max(ps(52), 14)}px "Noto Sans", "Helvetica", sans-serif`;
    const lastLineBaseline = wrapText(ctx, rulesText, textX, textCursor, textWidth, textLineHeight, textMaxY);
    textCursor = lastLineBaseline + textLineHeight;
  }
  ctx.restore();

  if (cardData.type === 'UNIT') {
    const minDiagramTop = Math.max(diagramTop, textCursor + Math.max(py(diagramsCfg.spacingFromText ?? 32), 18));
    const maxDiagramTop = py(layout.statsTop) - textAreaPadding - diagramHeight;
    diagramTop = Math.min(Math.max(diagramTop, minDiagramTop), maxDiagramTop);
  }
  ...
  const drawOrbNumber = (text, position, fontBase, fontFamily = 'Cinzel', weight = '700') => {
    ...
    ctx.shadowBlur = Math.max(ps(64), 12);
    ctx.shadowOffsetY = Math.max(ps(6), 2);
  ...
  if (cardData.type === 'UNIT') {
    ...
    drawOrbNumber(String(act), layout.activation, 100, 'Cinzel', '700');

    const hpToShow = (hpOverride != null) ? hpOverride : (cardData.hp || 0);
    const atkToShow = (atkOverride != null) ? atkOverride : (cardData.atk || 0);
    drawOrbNumber(String(hpToShow), layout.hp, 112, 'Noto Sans', '800');
    drawOrbNumber(String(atkToShow), layout.atk, 112, 'Noto Sans', '800');

    const cell = diagramCell ?? Math.max(Math.round(ps(diagramsCfg.cell ?? 28)), 10);
    const gap = diagramGap ?? Math.max(Math.round(ps(diagramsCfg.gap ?? 6)), 2);
...
function wrapText(ctx, text, x, y, maxWidth, lineHeight, maxY = Infinity) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (ctx.measureText(testLine).width > maxWidth && line) {
      ...
      lastBaseline = cursorY;
      drewLine = true;
      cursorY += lineHeight;
      if (cursorY > maxY) { line = ''; break; }
      line = word;
    } else {
      line = testLine;
    }
  ...
  return drewLine ? lastBaseline : y;
}

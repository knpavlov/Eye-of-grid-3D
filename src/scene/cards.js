...
const CARD_FACE_LAYOUT = {
  ...
  nameY: 152,
  typeY: 206,
  art: { x: 48, y: 276, width: 736, height: 432 },
  textGap: 32,
  textBottomGap: 22,
  diagramsBottom: 1158,
  diagramLabelGap: 26,
...
export function drawCardFace(ctx, cardData, width, height, hpOverride = null, atkOverride = null, opts = {}) {
  ...
  let nameFont = Math.max(ps(34), 16);
  ...
  const descFontPx = Math.max(ps(13), 9);
  ...
  if (cardData.type === 'UNIT') {
    diagramCell = Math.max(Math.round(ps(20.8)), 7);
  ...
  // Значения ресурсов
  const drawOrbNumber = (text, position, fontBase, fontFamily = 'Cinzel', weight = '700', minPx = 12) => {
    ctx.save();
    ctx.textAlign = 'center';
  ...
  if (cardData.type === 'UNIT') {
    ...
    drawOrbNumber(String(act), layout.activation, 67, 'Cinzel', '700', 11);

    const hpToShow = (hpOverride != null) ? hpOverride : (cardData.hp || 0);
    const atkToShow = (atkOverride != null) ? atkOverride : (cardData.atk || 0);
    drawOrbNumber(String(hpToShow), layout.hp, 50, 'Noto Sans', '800', 11);
    drawOrbNumber(String(atkToShow), layout.atk, 50, 'Noto Sans', '800', 11);

    const cell = diagramCell ?? Math.max(Math.round(ps(20.8)), 7);
    const gap = diagramGap ?? Math.max(Math.round(ps(4)), 2);
    const gridW = cell * 3 + gap * 2;
    const spacing = Math.max(Math.round(ps(38)), 11);

...
const CARD_FACE_LAYOUT = {
  ...
  cost: { x: 96, y: 94 },
  activation: { x: 236, y: 126 },
  hp: { x: 78, y: 1184 },
  atk: { x: 752, y: 1180 },
  lock: { x: 164, y: 174 },
  nameY: 222,
  typeY: 276,
  art: { x: 48, y: 304, width: 736, height: 432 },
  textGap: 32,
  textToDiagramGap: 32,
  diagramMinTop: 908,
  diagramBottom: 1196
...
export function drawCardFace(ctx, cardData, width, height, hpOverride = null, atkOverride = null, opts = {}) {
  ...
  let nameFont = Math.max(ps(48), 22);
  ...
  ctx.shadowOffsetY = Math.max(ps(3), 1);
  ...
  const baseDiagramBottom = layout.diagramBottom ?? (layout.hp ? layout.hp.y + 24 : BASE_H - 56);
  const diagramBottom = py(baseDiagramBottom);
  const diagramGapPx = Math.max(py(layout.textToDiagramGap || 32), 16);
  const minDiagramTop = layout.diagramMinTop ? py(layout.diagramMinTop) : diagramGapPx;
  let diagramTop = diagramBottom;
  ...
  if (cardData.type === 'UNIT') {
    ...
    diagramTop = diagramBottom - diagramHeight;
    diagramTop = Math.max(diagramTop, minDiagramTop);
    diagramTop = Math.max(diagramTop, textY + diagramGapPx);
    if (diagramTop + diagramHeight > diagramBottom) {
      diagramTop = Math.max(minDiagramTop, diagramBottom - diagramHeight);
    }
  }

  const textLineHeight = Math.max(ps(34), 14);
  const textMaxY = (cardData.type === 'UNIT')
    ? diagramTop - diagramGapPx
    : diagramBottom - diagramGapPx;
  wrapText(ctx, rulesText, textX, textY, textWidth, textLineHeight, textMaxY);
  ctx.restore();
  ...
  if (cardData.type === 'UNIT') {
    ...
    drawOrbNumber(String(act), layout.activation, 72, 'Cinzel', '700');

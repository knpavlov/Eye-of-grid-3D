...
function getTurnTimer() {
  try {
    return window.__ui?.turnTimer || null;
  } catch {
    return null;
  }
}
...
function updateTurnTimerPause(state) {
  if (!state) {
    resumeTurnTimer();
    return;
  }
  const queue = Array.isArray(state.pendingDiscards) ? state.pendingDiscards : [];
  const needsPause = queue.some(item => item && item.remaining > 0);
  if (needsPause) {
    pauseTurnTimer();
  } else {
    resumeTurnTimer();
...
function maybeScheduleAutoEndAfterDiscard(req, state) {
  ...
  if (!interactionState.pendingAutoEndAfterSummon) return;
  try {
    if (typeof window !== 'undefined') {
      window.__interactions?.requestAutoEndTurn?.({ reason: 'SUMMON' });
    }
  } catch {}
}
...
function finishRequest() {
  ...
  updateTurnTimerPause(typeof window !== 'undefined' ? window.gameState : null);
}

...
function setTurnTimerPaused(nextState, secondsOwnerState = null) {
  const desired = !!nextState;
  if (managerState.turnTimerPaused === desired) return;
  managerState.turnTimerPaused = desired;
  try {
    if (desired) {
      window.__ui?.turnTimer?.stop?.();
    } else {
      if (secondsOwnerState && typeof secondsOwnerState.turnTimerSeconds === 'number') {
        window.__ui?.turnTimer?.set?.(secondsOwnerState.turnTimerSeconds);
      }
      window.__ui?.turnTimer?.start?.();
    }
  } catch {}
}

function getQueue(state) {
  return Array.isArray(state?.pendingDiscards) ? state.pendingDiscards : [];
}

function updateTurnTimerState(state) {
  const queue = getQueue(state);
  const hasAny = queue.some(item => item && item.remaining > 0);
  const turnState = state || (typeof window !== 'undefined' ? window.gameState : null);
  const secondsOwnerState = turnState
    ? { turnTimerSeconds: typeof window !== 'undefined' ? window.__turnTimerSeconds : undefined }
    : null;
  setTurnTimerPaused(hasAny, secondsOwnerState);
}
...
function completeRequestAndSync(req, state) {
  if (!req) return;
  removeRequestFromState(req.id, { push: true });
  finishRequest();
  const gs = (typeof window !== 'undefined' ? window.gameState : state) || state;
  updateTurnTimerState(gs);
...
function finishRequest() {
  ...
  updateTurnTimerState(typeof window !== 'undefined' ? window.gameState : null);
}

function applyStateMutation(updater, { push = false } = {}) {
  try {
    const gs = window.gameState;
    if (!gs) return;
    updater(gs);
    if (push && typeof window.schedulePush === 'function') {
      window.schedulePush('forced-discard', { force: true });
    }
...
function removeRequestFromState(reqId, { push = false } = {}) {
...
function updateRequestRemaining(reqId, remaining, { push = false } = {}) {
  applyStateMutation((gs) => {
    const queue = Array.isArray(gs.pendingDiscards) ? gs.pendingDiscards : (gs.pendingDiscards = []);
...
function handleDiscard(handIdx, { auto = false } = {}) {
  ...
  updateRequestRemaining(req.id, newRemaining, { push: true });
...
export function isTurnTimerSuppressed() {
  return !!managerState.turnTimerPaused;
}

const api = { initDiscardManager, syncWithState, hasActiveLocalRequest, isTurnTimerSuppressed };

...
import { interactionState, requestAutoEndTurn } from '../scene/interactions.js';
...
function finishRequest(options = {}) {
  const current = managerState.active ? { ...managerState.active } : null;
  clearTimers();
  resetSelectionState();
  hideOverlay();
  managerState.active = null;
  if (options.completed && current && typeof window !== 'undefined') {
    try {
      const state = window.gameState;
      if (state && typeof current.target === 'number' && state.active === current.target) {
        // Если активный игрок выполнил все требования сброса, возвращаем авто-передачу хода
        requestAutoEndTurn();
      }
    } catch {}
  }
...
function handleDiscard(handIdx, { auto = false } = {}) {
  ...
  if (!player || !Array.isArray(player.hand) || player.hand.length === 0) {
    ...
    finishRequest({ completed: true, request: req });
    return;
  }

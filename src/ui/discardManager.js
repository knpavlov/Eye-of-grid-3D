...
// Возобновляем таймер, когда все запросы на сброс завершены
function resumeTurnTimerIfIdle(state) {
  ...
  const queue = getQueue(gs);
  const stillPending = queue.some(item => item && item.remaining > 0);
  if (stillPending) return;
  managerState.turnTimerPaused = false;
  try { window.__ui?.turnTimer?.start?.(); } catch {}
}

function hasPendingRequests(state, { target = null, excludeId = null } = {}) {
  if (!state) return false;
  const queue = getQueue(state);
  return queue.some((item) => {
    if (!item || item.remaining <= 0) return false;
    if (target !== null && item.target !== target) return false;
    if (excludeId !== null && item.id === excludeId) return false;
    return true;
  });
}

function ensureAutoEndResume(state) {
  const gs = state || (typeof window !== 'undefined' ? window.gameState : null);
  if (!gs) return;
  const pendingAuto = Math.max(0, Number(gs.pendingAutoEndRequests) || 0);
  if (pendingAuto <= 0) return;
  if (hasPendingRequests(gs)) return;
  const activeSeat = typeof gs.active === 'number' ? gs.active : null;
  if (activeSeat == null) return;
  const mySeatKnown = (typeof window !== 'undefined' && typeof window.MY_SEAT === 'number');
  if (mySeatKnown && window.MY_SEAT !== activeSeat) return;
  try {
...
export function syncWithState(state) {
  if (state && hasPendingRequests(state)) {
    pauseTurnTimer();
  } else {

// Minimal always-on status chip for diagnostic info

export function attach() {
  function mount() {
    if (document.getElementById('mod-status')) return;
    const el = document.createElement('div'); el.id = 'mod-status';
...
const api = { attach };
try { if (typeof window !== 'undefined') { window.__ui = window.__ui || {}; window.__ui.status = api; } } catch {}
attach();
export default api;

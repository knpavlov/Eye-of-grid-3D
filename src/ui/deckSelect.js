...
export function open(onConfirm, onCancel) {
  ...
  panel.className = 'bg-slate-800 p-4 rounded-lg w-[26rem] max-h-[90vh] flex flex-col';
  ...
  DECKS.forEach((d, idx) => {
    ...
    item.className = 'relative flex h-24 cursor-pointer rounded-md border border-slate-700 overflow-hidden transition transform hover:scale-[1.02] hover:shadow-lg';
    if (idx === selected) item.classList.add('ring-2', 'ring-yellow-500', 'ring-inset');
  ...
  cancelBtn.className = 'overlay-panel px-3 py-1.5 bg-slate-600 hover:bg-slate-700 glossy-btn';
  cancelBtn.textContent = 'Cancel';
  cancelBtn.addEventListener('click', () => { document.body.removeChild(overlay); onCancel && onCancel(); });
  btnWrap.appendChild(cancelBtn);

  const okBtn = document.createElement('button');
  okBtn.className = 'overlay-panel px-3 py-1.5 bg-slate-600 hover:bg-slate-700 glossy-btn';

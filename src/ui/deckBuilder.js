...
import { drawCardFace, preloadCardTextures, subscribeCardFaceUpdates } from '../scene/cards.js';
...
// Соотношение сторон и размер превью карт для сетки 5х2
const CARD_ASPECT = 356 / 256;
const PREVIEW_W = 176;
const PREVIEW_H = Math.round(PREVIEW_W * CARD_ASPECT);
...
export function open(deck = null, onDone) {
  ...
  overlay.className = 'fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 overflow-auto';

  const panel = document.createElement('div');
  panel.className = 'bg-slate-800 mt-4 p-5 rounded-lg w-[94vw] h-[96vh] flex flex-col relative shadow-2xl deck-builder-panel';
  ...
  deckList.className = 'flex-1 overflow-y-auto space-y-1 text-sm deck-scroll pr-1';
  ...
  // Сетка 5x2 с кастомным скроллом
  catalog.className = 'flex-1 overflow-y-auto grid grid-cols-5 grid-rows-2 gap-4 pl-6 pr-2 deck-scroll catalog-grid';
  ...
  function renderDeck() {
    ...
    Object.values(grouped)
      ...
      .forEach(({ card, count }) => {
        ...
        row.className = 'deck-entry relative rounded overflow-hidden cursor-pointer';
        const stripCfg = card.deckStrip || {};
        const stripHeight = typeof stripCfg.height === 'number'
          ? stripCfg.height
          : (typeof card.deckStripHeight === 'number' ? card.deckStripHeight : STRIP_HEIGHT);
        const stripOffset = typeof stripCfg.offset === 'number'
          ? stripCfg.offset
          : (typeof card.deckStripOffset === 'number' ? card.deckStripOffset : STRIP_OFFSET);
        const stripAlign = stripCfg.align || card.deckStripAlign || 'center';
        ...
        row.style.backgroundPosition = `${stripAlign} ${stripOffset}%`;
  ...
  function redrawPreviews() {
    previewEntries.forEach(({ card, canvas }) => {
      const ctx = canvas.getContext('2d');
      if (!ctx) return;
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      drawCardFace(ctx, card, canvas.width, canvas.height);
    });
  }

  const unsubscribeFaces = typeof subscribeCardFaceUpdates === 'function'
    ? subscribeCardFaceUpdates(redrawPreviews)
    : null;
  ...
  function renderCatalog() {
    ...
    cards.forEach(card => {
      ...
      previewEntries.add({ card, canvas });
    });
  }
  ...
  function cleanup() {
    ...
    if (typeof unsubscribeFaces === 'function') unsubscribeFaces();
  }

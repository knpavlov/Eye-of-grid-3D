...
function createLimitPopup(host) {
  ...
  // Убеждаемся, что контейнер является относительным, чтобы позиционировать оверлей
  if (!host.style.position || host.style.position === '') {
    host.style.position = 'relative';
  }

  // Создаём оверлей поверх каталога, который не сдвигает контент
  const wrapper = document.createElement('div');
  wrapper.className = 'pointer-events-none absolute inset-0 flex justify-center pt-6 transition-opacity duration-200';
...
export function open(deck = null, onDone) {
  ...
  let limitPopup = { show() {}, hide() {}, dispose() {} };
  ...
  catalogContainer.className = 'relative flex-1 flex';
  right.appendChild(catalogContainer);
  ...
  limitPopup = createLimitPopup(catalogContainer);

...
export function open(initial = false) {
  ...
  addBtn('mm-online', 'Play Online', () => {
    ...
    const startOnline = deck => {
      ...
      // сохраняем выбранную колоду глобально, чтобы сервер знал наш выбор
      window.__selectedDeckObj = deck;
      try { window.__myDeckId = deck.id; window.__oppDeckId = null; } catch {}
      firstOpen = false;
  ...
  addBtn('mm-offline', 'Play Offline', () => {
    close();
    const ds = window.__ui?.deckSelect;
    // перед запуском офлайн-матча гарантированно выходим из сетевой игры/очереди
    try { window.socket?.emit('leaveQueue'); } catch {}
    try { window.socket?.emit('resign'); } catch {}
    try { window.socket?.disconnect(); } catch {}
    try { window.NET_ACTIVE = false; window.updateIndicator?.(); window.updateInputLock?.(); } catch {}

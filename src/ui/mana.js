...
// Плавающая подпись с количеством полученной маны
function playManaAmountLabel(start, { amount = 1, durationMs = 700, riseHeight = 140, lingerMs = 320 } = {}) {
  if (!start) return null;
  ...
  label.style.transform = 'translate(-50%, -50%) translateY(0) scale(0.8)';
  ...
  const rise = Math.max(40, riseHeight * 0.75);
  const linger = Math.max(120, lingerMs);
  ...
  if (tl) {
    ...
    const appear = Math.min(0.18, total * 0.3);
    const travel = total;
    const fade = Math.max(0.22, (linger + 180) / 1000);
    ...
    tl.to(label, {
      ...
      .to(label, {
        ...
        ease: 'power1.out',
        y: -rise,
      }, `>-=${Math.min(0.08, appear * 0.4)}`)
      .to(label, {
        duration: fade,
        ease: 'power2.in',
        opacity: 0,
        y: `-=${rise * 0.2}`,
      }, `>${Math.max(0.08, linger / 1000)}`);
  } else {
    label.style.transition = `transform ${duration}ms ease-out, opacity 220ms ease-out`;
    requestAnimationFrame(() => {
      label.style.opacity = '1';
      label.style.transform = `translate(-50%, -50%) translateY(-${rise}px) scale(1)`;
      setTimeout(() => {
        label.style.transition = 'opacity 220ms ease-out';
        label.style.opacity = '0';
...
export function animateManaGainFromWorld(pos, ownerIndex, visualOnly = true, targetSlotMaybe = null, optionsMaybe = {}) {
  try {
    ...
    const manaAmount = Math.max(1, Math.floor(opts.manaAmount ?? opts.count ?? 1));
    const labelLingerMs = Math.max(120, Number.isFinite(opts.labelLingerMs) ? Number(opts.labelLingerMs) : 320);

    const preferredRaw = [];
    if (Array.isArray(opts.panelSlots)) preferredRaw.push(...opts.panelSlots);
    if (Array.isArray(opts.slotSequence)) preferredRaw.push(...opts.slotSequence);
    if (Array.isArray(opts.slots)) preferredRaw.push(...opts.slots);
    if (targetSlot != null) preferredRaw.unshift(targetSlot);
    const preferredSlots = [];
    for (const raw of preferredRaw) {
      const normalized = clampIndex(raw);
      if (normalized == null) continue;
      if (preferredSlots.includes(normalized)) continue;
      preferredSlots.push(normalized);
    }

    const takenSlots = new Set();
    const gameState = (typeof window !== 'undefined') ? window.gameState : null;

    const determineSlot = (seqIdx = 0) => {
      const predefined = (seqIdx < preferredSlots.length) ? preferredSlots[seqIdx] : null;
      if (predefined != null && !takenSlots.has(predefined)) {
        takenSlots.add(predefined);
        return predefined;
      }
      ...
      if (!barEl) {
        const fallback = Math.max(0, Math.min(9, currentMana - 1));
        if (!takenSlots.has(fallback)) takenSlots.add(fallback);
        return fallback;
      }
      ...
      try {
        ...
        const planned = filledNow + pending.size + anonymousPending + takenSlots.size;
        resolved = Math.max(0, Math.min(9, planned));
      } catch {
        const visibleMana = Math.max(0, currentMana - currentBlocks + takenSlots.size);
        resolved = Math.max(0, Math.min(9, visibleMana));
      }

      if (resolved == null) return null;
      if (!takenSlots.has(resolved)) {
        takenSlots.add(resolved);
        return resolved;
      }

      let candidate = resolved;
      for (let attempts = 0; attempts < 10; attempts += 1) {
        candidate = Math.max(0, Math.min(9, candidate + 1));
        if (!takenSlots.has(candidate)) {
          takenSlots.add(candidate);
          return candidate;
        }
    ...
    const reserveSlots = () => {
      ...
      try {
        if (preferredSlots.length > 0) {
          reserveIncomingOrbs(ownerIndex, preferredSlots, manaAmount);
        } else {
          reserveIncomingOrbs(ownerIndex, [], manaAmount);
        }
      } catch {}
    };

    const releaseReservedOnFailure = () => {
      if (!reserved || !visualOnly || !Number.isFinite(ownerIndex)) return;
      try {
        if (preferredSlots.length > 0) {
          for (const slot of preferredSlots) {
            releaseIncomingOrb(ownerIndex, slot);
          }
          const leftovers = Math.max(0, manaAmount - preferredSlots.length);
    ...
    if (visualOnly && Number.isFinite(ownerIndex)) {
      if (reserveImmediately) {
        ...
        const reserveLead = Math.max(0, startDelayMs - 120);
    ...
    const runEffect = () => {
      try {
        ...
        playManaAmountLabel(start, {
          amount: manaAmount,
          durationMs: flightDurationMs,
          riseHeight,
          lingerMs: labelLingerMs,
        });
        ...
        setTimeout(() => {
          try {
            const revealSlots = [];
            for (let i = 0; i < manaAmount; i += 1) {
              const slot = determineSlot(i);
              if (slot != null) revealSlots.push(slot);
            }
            if (!revealSlots.length) {
              releaseReservedOnFailure();
            } else {
              for (const slot of revealSlots) {
                playPanelOrbReveal(ownerIndex, slot, { visualOnly });
              }
              if (visualOnly && Number.isFinite(ownerIndex) && preferredSlots.length > revealSlots.length) {
                const used = new Set(revealSlots);
                for (const slot of preferredSlots) {
                  if (!used.has(slot)) {
                    releaseIncomingOrb(ownerIndex, slot);
                  }
                }
                const diff = Math.max(0, manaAmount - revealSlots.length);
            ...
            console.error('[mana] Ошибка появления орбов на панели:', err);
            releaseReservedOnFailure();
          } finally {
            finalizeFlight();
          }

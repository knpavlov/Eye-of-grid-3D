...
function queueSparkBurst(tl, centerX, centerY, count = 14, startAt = 0.04, recordSpark) {
  if (!tl || typeof centerX !== 'number' || typeof centerY !== 'number') return;
  const baseStart = Math.max(0, startAt);
  for (let i = 0; i < count; i += 1) {
    const spark = document.createElement('div');
    spark.className = 'mana-orb-spark';
    spark.style.left = `${centerX}px`;
    spark.style.top = `${centerY}px`;
    spark.style.opacity = '0';
    document.body.appendChild(spark);
    if (typeof recordSpark === 'function') recordSpark(spark);
    ...
    tl.set(spark, { x: 0, y: 0, opacity: 0, scale: 0.45 }, baseStart - 0.02 + jitter);
    tl.to(spark, {
      x: dx,
      y: dy,
      opacity: 1,
    ...
    tl.to(spark, {
      ...
      duration: 0.32,
      ease: 'power1.in'
    }, baseStart + 0.18 + jitter);
  }
}

function queueManaBarFlash(tl, bar, startAt = 0) {
  if (!tl || !bar) return;
  const time = Math.max(0, startAt);
  tl.to(bar, {
...
function queuePanelOrbReveal(tl, el, {
  bar = null,
  startAt = 0,
  sparkCount = 16,
  arrivalScale = 2.35,
  ...
  tl.call(() => {
    try {
      ...
      el.style.boxShadow = '0 0 12px rgba(30,160,255,0.8)';
    } catch {}
  }, null, time);
  ...
  tl.to(el, {
    ...
    onStart: () => {
      el.style.opacity = '1';
      el.style.boxShadow = '0 0 26px rgba(125,211,252,0.95), 0 0 36px rgba(56,189,248,0.85)';
    },
    onComplete: () => {
      el.style.boxShadow = '0 0 14px rgba(56,189,248,0.82)';
    }
  }, time);
  ...
  try {
    ...
    queueSparkBurst(tl, cx, cy, sparkCount, time + 0.05, recordSpark);
  } catch {}

  if (bar) {
    queueManaBarFlash(tl, bar, time);
  }
}

function playPanelOrbArrival(ownerIndex, slotIndex, {
  delayMs = 0,
  sparkCount = 14,
  flashBar = true,
  ...
  const owner = Number.isFinite(ownerIndex) ? Math.max(0, Math.min(1, Math.floor(ownerIndex))) : 0;
  const slot = clampIndex(slotIndex);

  const finalize = () => {
    if (releaseOnComplete && Number.isFinite(ownerIndex)) {
      releaseIncomingOrb(owner, slot);
    }
  ...
  const run = () => {
    try {
      const bar = document.getElementById(`mana-display-${owner}`);
      if (!bar) {
        finalize();
        return;
      }
      const el = (slot != null && bar.children) ? bar.children[slot] : null;
      if (!el) {
        finalize();
        return;
      }
      ...
      queuePanelOrbReveal(tl, el, {
        bar: flashBar ? bar : null,
        sparkCount,
        recordSpark: spark => sparks.push(spark),
      });
    } catch (err) {
      console.error('[mana] Ошибка при появлении орба на панели:', err);
      finalize();
    }
  };
...
export function animateManaGainFromWorld(pos, ownerIndex, visualOnly = true, targetSlotMaybe = null, optionsMaybe = {}) {
  try {
    ...
    const flightDurationMs = Math.max(200, Number.isFinite(opts.flightDurationMs) ? Number(opts.flightDurationMs) : 500);
    const sparkCount = Math.max(4, Math.floor(opts.sparkCount ?? 14));
    const flashBar = opts.flashBar !== false;
    ...
    const scheduleSingle = (slotIndex, extraDelayMs) => {
      ...
      setTimeout(() => {
        ...
        try {
          ...
          const start = source ? worldToScreen(source) : null;
          const barEl = document.getElementById(`mana-display-${ownerIndex}`);

          if (resolvedSlot == null && barEl) {
            try {
              const pendingSet = getPendingSlotSet(ownerIndex);
              for (let idx = 0; idx < Math.min(10, barEl.children.length); idx += 1) {
                const child = barEl.children[idx];
                const filled = child?.classList?.contains('mana-orb') || child?.dataset?.filled === '1';
                if (pendingSet.has(idx)) continue;
                if (!filled) {
                  resolvedSlot = idx;
                  break;
                }
              }
              if (resolvedSlot == null && barEl.children.length) {
                resolvedSlot = Math.min(9, barEl.children.length - 1);
              }
            } catch {}
          }
          ...
          if (resolvedSlot == null) {
            ...
            resolvedSlot = clampIndex(Math.max(0, Math.min(9, Math.floor(currentMana))));
          }
          ...
          const triggerPanelArrival = () => {
            if (visualOnly) {
              playPanelOrbArrival(ownerIndex, resolvedSlot, {
                sparkCount,
                flashBar,
              });
          ...
          orb.className = 'mana-flight-orb';
          orb.style.left = `${start.x}px`;
          orb.style.top = `${start.y}px`;
          orb.style.transform = 'translate(-50%, -50%) scale(0.45)';
          orb.style.opacity = '0';
          ...
          if (tl) {
            tl.to(orb, {
              ...
              .to(orb, {
                duration: Math.max(0.2, (flightDurationMs / 1000) * 0.9),
                ease: 'power1.in',
                opacity: 0,
              }, '<+0.05');
          } else {
            requestAnimationFrame(() => {
              try {
                orb.style.transition = `transform ${flightDurationMs}ms cubic-bezier(.22,.61,.36,1), opacity ${flightDurationMs}ms ease-in`;
                orb.style.opacity = '1';
                orb.style.transform = 'translate(-50%, calc(-50% - 110px)) scale(1.05)';
              } catch {}
            });
            setTimeout(cleanup, flightDurationMs + 60);
          }
        } catch (err) {

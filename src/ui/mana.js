...
// Экспортируемая обёртка, чтобы сценические планировщики могли заранее
// забронировать слоты и избежать мерцания орбов при обновлении UI.
export function reserveIncomingMana(ownerIndex, amount) {
  reserveIncomingBlocks(ownerIndex, amount);
}
...
export function animateManaGainFromWorld(pos, ownerIndex, visualOnly = true, targetSlotMaybe = null, optionsMaybe = {}) {
  try {
    ...
    const delayBetweenMs = Math.max(0, Number.isFinite(opts.delayBetweenMs) ? Number(opts.delayBetweenMs) : 180);
...
const api = { renderBars, animateManaGainFromWorld, animateTurnManaGain, reserveIncomingMana };

...
// Мягкая подсветка панели маны при мгновенном появлении орбов
function flashManaBar(bar){
  if (!bar) return;
  try {
    ...
    if (hasGsap) {
      ...
      tl.to(bar, {
        filter: 'brightness(2.05) drop-shadow(0 0 18px rgba(56,189,248,0.85))',
        duration: 0.18,
        ease: 'power2.out'
      }).to(bar, {
        filter: 'none',
      ...
      bar.style.transition = 'filter 320ms ease';
...
// Создание блесток вокруг орба
function spawnOrbSparks(orb, count = 14){
  if (!orb || typeof document === 'undefined') return [];
  try {
...
// Анимация орба и блесток при мгновенном появлении
function animateInstantOrbReveal(orb){
  if (!orb) return;
  try {
    orb.className = 'mana-orb';
    orb.style.opacity = '0';
    orb.style.transform = 'scale(0.58)';
    orb.style.willChange = 'transform, opacity, box-shadow';
    const sparks = spawnOrbSparks(orb);
    const cleanupSparks = () => {
      for (const s of sparks) {
        try { if (s.parentNode) s.parentNode.removeChild(s); } catch {}
      }
    };
    const hasGsap = (typeof window !== 'undefined') && window.gsap && typeof window.gsap.timeline === 'function';
    if (hasGsap) {
      const tl = window.gsap.timeline({ onComplete: cleanupSparks });
      tl.to(orb, {
        duration: 0.2,
        ease: 'back.out(2.2)',
        scale: 2.4,
        opacity: 1,
        onStart: () => {
          orb.style.boxShadow = '0 0 22px rgba(96,165,250,0.95), 0 0 40px rgba(56,189,248,0.8)';
        }
      }, 0)
        .to(orb, {
          duration: 0.36,
          ...
          scale: 1,
          onComplete: () => {
            orb.style.boxShadow = '0 0 12px rgba(30,160,255,0.85)';
            orb.style.willChange = '';
          }
        }, 0.2);

      const sparkDuration = 0.46;
      sparks.forEach((spark, idx) => {
        const angle = (Math.PI * 2) * (idx / sparks.length) + Math.random() * 0.7;
        const dist = 36 + Math.random() * 42;
        const dx = Math.cos(angle) * dist;
        const dy = Math.sin(angle) * dist;
        tl.fromTo(spark,
          { x: 0, y: 0, opacity: 0, scale: 0.6 },
          { x: dx, y: dy, opacity: 1, scale: 1.1, duration: 0.18, ease: 'power2.out' },
          0.04)
          .to(spark, {
            opacity: 0,
            scale: 0.3,
      ...
      const distList = sparks.map((_, idx) => {
        const angle = (Math.PI * 2) * (idx / Math.max(1, sparks.length)) + Math.random() * 0.7;
        const dist = 36 + Math.random() * 42;
        return {
      ...
      requestAnimationFrame(() => {
        try {
          orb.style.transition = 'transform 200ms ease-out, opacity 180ms ease-out';
          orb.style.opacity = '1';
          orb.style.transform = 'scale(2.4)';
          orb.style.boxShadow = '0 0 22px rgba(96,165,250,0.95), 0 0 40px rgba(56,189,248,0.8)';
          setTimeout(() => {
            orb.style.transition = 'transform 360ms ease-in-out, opacity 320ms ease-in';
            orb.style.transform = 'scale(1)';
            orb.style.boxShadow = '0 0 12px rgba(30,160,255,0.85)';
            setTimeout(() => { orb.style.willChange = ''; }, 360);
          }, 200);
        } catch {}
      });
      requestAnimationFrame(() => {
        sparks.forEach((spark, idx) => {
          try {
            spark.style.transition = 'transform 220ms ease-out, opacity 180ms ease-out';
            spark.style.opacity = '1';
            spark.style.transform = `translate(${distList[idx].dx}px, ${distList[idx].dy}px) scale(1.15)`;
            setTimeout(() => {
              spark.style.transition = 'transform 320ms ease-in, opacity 280ms ease-in';
              spark.style.opacity = '0';
              spark.style.transform = `translate(${distList[idx].dx}px, ${distList[idx].dy}px) scale(0.3)`;
            }, 200);
          } catch {}
        });
      });
      setTimeout(cleanupSparks, 720);
    }
  } catch (err) {
    console.error('[mana] Ошибка анимации мгновенного орба:', err);
  }
}
...
export function renderBars(gameState) {
  ...
  for (let p = 0; p < 2; p++) {
    ...
    if (revealRange && revealMatched) {
      if (animateAllowed) {
        flashManaBar(manaDisplay);
        for (const node of revealTargets) {
          animateInstantOrbReveal(node);
        }
      }
      shiftReveal(p);
    }
      ...
      try {
        ...
        if (tl) {
          ...
          tl.to(container, { opacity: 1, scale: 1, duration: 0.22, ease: 'back.out(1.8)' })
            ...
            .to(label, { opacity: 1, y: -12, duration: 0.28, ease: 'power2.out' }, '<+0.06')
            .to(label, { y: -24, duration: Math.max(0.12, floatSeconds + holdSeconds - 0.28), ease: 'power1.out' }, '<')
            .call(applyReveal, null, `<+${Math.max(0.01, floatSeconds)}`)
          ...
          setTimeout(() => {
            try {
              label.style.transition = 'opacity 240ms ease, transform 320ms ease';
              label.style.opacity = '1';
              label.style.transform = 'translate(-50%, -120%) translateY(-12px)';
            } catch {}
          }, 90);
          setTimeout(() => {
            try {
              label.style.transition = 'opacity 260ms ease, transform 320ms ease';
              label.style.opacity = '0';
              label.style.transform = 'translate(-50%, -120%) translateY(-24px)';
            } catch {}
          }, floatDuration * 1000 + labelHoldMs);
          setTimeout(() => { try { flash.style.opacity = '1'; } catch {} }, Math.max(0, floatDuration * 1000 - 160));
          setTimeout(() => { applyReveal(); }, Math.round(floatDuration * 1000));

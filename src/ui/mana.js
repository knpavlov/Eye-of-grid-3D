...
// Расчёт момента, когда панель должна раскрыться, относительно полёта орба
function computeRevealMoment(floatSeconds, fadeSeconds) {
  const flight = Math.max(0, Number(floatSeconds) || 0);
  const dissolve = Math.max(0, Number(fadeSeconds) || 0);
  const anticipation = Math.min(0.32, Math.max(0.12, flight * 0.45));
  const dissolveWindow = Math.min(0.34, dissolve || 0);
  const revealAt = Math.max(0.08, flight - dissolveWindow - anticipation);
  return revealAt;
}
...
export function renderBars(gameState) {
  ...
  for (let p = 0; p < 2; p++) {
    ...
    // Пропускаем перерисовку только для панели, которая сейчас раскрывается
    ...
    if (pending || (getManaGainActive() && (!anim || anim.ownerIndex === p))) {
      continue;
    }
...
function populatePanelBurst(tl, bar, indices, sparks, { highlight = true } = {}) {
    ...
    setTimeout(() => {
          ...
          const scheduleEffect = () => {
            try {
              ...
              if (bar && typeof window !== 'undefined' && window.gsap?.timeline) {
                const sparks = [];
                const tl = window.gsap.timeline({ onComplete: () => cleanupSparks(sparks) });
                populatePanelBurst(tl, bar, indices, sparks, { highlight: true });
              }
            } catch {}
          };
      ...
      try {
        ...
        if (tl) {
          ...
          const revealMoment = computeRevealMoment(floatSeconds, fadeSeconds);
          ...
          const revealMoment = computeRevealMoment(floatDuration, fadeOutMs / 1000);
          ...
          setTimeout(() => { try { flash.style.opacity = '1'; } catch {} }, Math.max(0, floatMs - 160));
          setTimeout(() => { triggerReveal(); }, Math.max(0, revealMoment * 1000));

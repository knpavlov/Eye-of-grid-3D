...
export function renderBars(gameState) {
  ...
  for (let p = 0; p < 2; p++) {
    ...
    for (let i = 0; i < total; i += 1) {
      ...
      const skipAutoAnim = slot.dataset?.ignoreNextAutoAnim === '1';
      if (skipAutoAnim) {
        slot.dataset.ignoreNextAutoAnim = '0';
        slot.style.transition = '';
      } else if (animateAllowed && isFilled && !wasFilled) {
...
// Создаём визуальный орб-душу, который вылетает из существа
function createWorldSoulOrbElement(startX, startY) {
  try {
    const orb = document.createElement('div');
    orb.className = 'mana-orb-world';
    orb.style.position = 'fixed';
    orb.style.left = `${startX}px`;
    orb.style.top = `${startY}px`;
    orb.style.transform = 'translate(-50%, -50%) scale(0.4)';
    orb.style.opacity = '0';
...
// Анимируем вертикальный взлёт и затухание орба-души
function playWorldSoulOrbFlight(orb, durationMs, liftPx, easeName, onComplete) {
  if (!orb) {
  ...
  const lift = Number.isFinite(liftPx) ? liftPx : 120;
  const ease = typeof easeName === 'string' ? easeName : 'power2.in';
  ...
  if (tl) {
    tl.to(orb, {
      ...
      .to(orb, {
        ...
        y: -Math.abs(lift),
        opacity: 0,
        filter: 'drop-shadow(0 0 26px rgba(96,165,250,0.9))',
        onStart: () => { orb.style.mixBlendMode = 'screen'; }
      }, '>-0.06');
  } else {
    orb.style.transition = `transform ${durationSec}s ease-in, opacity ${durationSec}s ease-in`;
    orb.style.opacity = '1';
    orb.style.transform = `translate(-50%, calc(-50% - ${Math.abs(lift)}px)) scale(0.82)`;
    setTimeout(cleanup, durationMs + 160);
  }
}

// Подсвечиваем появление орба на панели вспышкой
function spawnPanelSpark(rect, durationMs = 420) {
  try {
    ...
    const tl = (typeof window !== 'undefined') ? window.gsap?.timeline?.({ onComplete: cleanup }) : null;
    if (tl) {
      tl.to(spark, { duration: 0.08, opacity: 1, ease: 'power1.out' })
        .to(spark, {
          duration: Math.max(0.14, durationMs / 1000),
          ease: 'power1.out',
          transform: 'translate(-50%, -50%) scale(1.9)',
          opacity: 0
        }, '>-0.02');
    } else {
      spark.style.transition = `transform ${durationMs}ms ease-out, opacity ${durationMs}ms ease-out`;
      requestAnimationFrame(() => {
        spark.style.opacity = '1';
        spark.style.transform = 'translate(-50%, -50%) scale(1.9)';
      });
...
// Запускаем анимацию появления орба в панели маны
function playPanelOrbReveal(ownerIndex, slotIndex) {
  try {
    const barEl = document.getElementById(`mana-display-${ownerIndex}`);
    if (!barEl) return;
    const total = 10;
    const slots = ensureManaSlots(barEl, total);
    const idx = clampIndex(slotIndex) ?? 0;
    const el = slots[idx];
    if (!el) return;

    el.dataset.ignoreNextAutoAnim = '1';
    ...
    el.style.boxShadow = '0 0 22px rgba(96,165,250,0.95)';
    el.style.willChange = 'transform, opacity, box-shadow';

    const rect = el.getBoundingClientRect();
    spawnPanelSpark(rect);
    ...
    if (tl) {
      tl.to(el, {
        ...
        ease: 'back.out(2.2)',
        opacity: 1,
        onStart: () => { el.style.boxShadow = '0 0 28px rgba(125,211,252,0.95)'; }
      })
        .to(el, { scale: 1.32, duration: 0.18, ease: 'power2.out' }, '<')
        .to(el, { scale: 1.0, duration: 0.32, ease: 'power2.inOut' });
    } else {
      requestAnimationFrame(() => {
        el.style.transition = 'transform 280ms ease-out, opacity 160ms ease-out';
        el.style.opacity = '1';
    ...
    console.error('[mana] Ошибка при анимации появления орба на панели:', err);
  }
}
...
export function animateManaGainFromWorld(pos, ownerIndex, visualOnly = true, targetSlotMaybe = null, optionsMaybe = {}) {
  try {
    ...
    const flightDuration = Math.max(0.3, Number.isFinite(opts.flightDuration) ? Number(opts.flightDuration) : 0.5);
    const soulDurationMs = Math.max(160, flightDuration * 1000);
    const flightEase = typeof opts.flightEase === 'string' ? opts.flightEase : 'power2.in';
    ...
    const scheduleSingle = (slotIndex, extraDelayMs) => {
      ...
      if (visualOnly && reserveImmediately && Number.isFinite(ownerIndex)) {
        ...
        setTimeout(() => {
          try {
            reserveIncomingOrbs(ownerIndex, normalizedSlot != null ? [normalizedSlot] : [], 1);
          } catch {}
        }, reserveLead);
      }
      setTimeout(() => {
        try {
          ...
          const orb = createWorldSoulOrbElement(start.x, start.y);
          let finalizeCalled = false;
          const finalize = () => {
            if (finalizeCalled) return;
            finalizeCalled = true;
            try { if (orb && orb.parentNode) orb.parentNode.removeChild(orb); } catch {}
            try { playPanelOrbReveal(ownerIndex, resolvedSlot); } catch (innerErr) {
              console.error('[mana] Ошибка финальной анимации маны:', innerErr);
            }
          ...
          playWorldSoulOrbFlight(orb, soulDurationMs, 118, flightEase, finalize);
          setTimeout(finalize, soulDurationMs + 40);
        } catch (err) {

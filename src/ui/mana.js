...
// Создаём визуальный контейнер для орба-души и подписи с количеством маны
function createSoulOrbElement(start, manaAmount = null) {
  const wrap = document.createElement('div');
  wrap.className = 'mana-soul-wrap';
  ...
  if (Number.isFinite(manaAmount) && manaAmount > 0) {
    ...
    label.textContent = `+${manaAmount}`;
    wrap.appendChild(label);
  }

  return { wrap, orb, label };
}

// Анимация стремительного вылета орба вверх и затухания
function playSoulOrbRiseEffect(start, { durationMs = 500, riseHeight = 140, manaAmount = null, labelHoldMs = 260 } = {}) {
  if (!start) return null;
  const elements = createSoulOrbElement(start, manaAmount);
  if (!elements || !elements.wrap) return null;
  document.body.appendChild(elements.wrap);

  const cleanup = () => {
    try { if (elements.wrap.parentNode) elements.wrap.parentNode.removeChild(elements.wrap); } catch {}
  };
  ...
  if (tl) {
    ...
    tl.set(elements.wrap, { opacity: 1 });
    ...
    if (elements.label) {
      const hold = Math.max(0.18, labelHoldMs / 1000);
    ...
    wrap.style.transition = `transform ${duration}ms ease-in, opacity 180ms ease-out`;
    orb.style.transition = `transform ${duration}ms ease-in, opacity ${duration}ms ease-in`;
    requestAnimationFrame(() => {
      wrap.style.opacity = '1';
      wrap.style.transform = `translate(-50%, -50%) translate(${drift}px, -${rise}px)`;
      orb.style.opacity = '1';
      orb.style.transform = `scale(1.25)`;
      if (label) {
        label.style.transition = `transform ${duration}ms ease, opacity ${duration}ms ease`;
        label.style.opacity = '1';
        label.style.transform = 'translateY(-12px)';
        setTimeout(() => {
          label.style.opacity = '0';
          label.style.transform = 'translateY(-36px)';
        }, Math.max(180, duration - labelHoldMs));
      }
      setTimeout(cleanup, duration + Math.max(220, labelHoldMs));
    });
  }
...
export function animateManaGainFromWorld(pos, ownerIndex, visualOnly = true, targetSlotMaybe = null, optionsMaybe = {}) {
  try {
    ...
    const requestedCount = Number.isFinite(opts.count) ? Number(opts.count) : null;
    const manaAmount = Math.max(1, Math.floor(Number.isFinite(opts.manaAmount) ? opts.manaAmount : (requestedCount ?? 1)));
    const startDelayMs = Math.max(0, Number.isFinite(opts.startDelayMs) ? Number(opts.startDelayMs) : 0);
    const flightDurationMs = Math.max(360, Number.isFinite(opts.flightDurationMs)
    ...
    const riseHeight = Number.isFinite(opts.riseHeight) ? Number(opts.riseHeight) : 150;
    const labelHoldMs = Math.max(180, Number.isFinite(opts.labelHoldMs) ? Number(opts.labelHoldMs) : 340);
    const reserveImmediately = !!opts.reserveImmediately;
    const slotSequenceRaw = Array.isArray(opts.slotSequence)
      ? opts.slotSequence
      : null;
    const panelSlotsRaw = Array.isArray(opts.panelSlots)
      ? opts.panelSlots
      : null;

    const sanitizeSlotArray = arr => (Array.isArray(arr)
      ? arr.map(raw => {
    ...
    let plannedSlots = sanitizeSlotArray(panelSlotsRaw);
    if (!plannedSlots.length) {
      plannedSlots = sanitizeSlotArray(slotSequenceRaw);
    }

    const baseSlot = Number.isFinite(targetSlot) ? clampIndex(targetSlot) : null;
    if (!plannedSlots.length && baseSlot != null) {
      for (let i = 0; i < manaAmount; i += 1) {
        const idx = clampIndex(baseSlot + i);
        if (idx != null) plannedSlots.push(idx);
      }
    }

    const revealCount = Math.max(plannedSlots.length, manaAmount);
    const gameState = (typeof window !== 'undefined') ? window.gameState : null;

    const determineFallbackSlots = () => {
      if (!visualOnly) {
        const currentMana = Math.max(0, (gameState?.players?.[ownerIndex]?.mana) || 0);
        const result = [];
        for (let i = 0; i < revealCount; i += 1) {
          result.push(Math.max(0, Math.min(9, currentMana - 1 - i)));
        }
        return result;
      }
      ...
      try {
        ...
        const slots = [];
        for (let i = 0; i < revealCount; i += 1) {
          slots.push(Math.max(0, Math.min(9, planned + i)));
        }
        return slots;
      } catch {
        return [];
    ...
    const reserveAll = () => {
      if (!visualOnly || !Number.isFinite(ownerIndex) || hasReserved) return;
      const slotsForReserve = plannedSlots.length ? plannedSlots : [];
      reserveIncomingOrbs(ownerIndex, slotsForReserve, revealCount);
      hasReserved = true;
    };
    ...
    const releaseAll = () => {
      ...
      if (plannedSlots.length) {
        ...
        for (let i = 0; i < revealCount; i += 1) {
          releaseIncomingOrb(ownerIndex, null);
        }
      }
      hasReserved = false;
    };
    ...
    let finalized = false;
    const finalizeFlight = (didReveal = false) => {
      if (finalized) return;
      finalized = true;
      if (!didReveal) {
        releaseAll();
    ...
    const runEffect = () => {
      try {
        ...
        const effectSlots = plannedSlots.length ? plannedSlots : determineFallbackSlots();
        const revealTargets = effectSlots.length ? effectSlots : new Array(revealCount).fill(null);
        ...
        setTimeout(() => {
          try {
            for (const slot of revealTargets) {
    ...
    if (visualOnly && Number.isFinite(ownerIndex) && !reserveImmediately && startDelayMs > 0) {
      const reserveLead = Math.max(0, startDelayMs - 140);
      if (reserveLead > 0) {
        setTimeout(() => {
          try { reserveAll(); } catch {}
        }, reserveLead);
      }
    } else if (reserveImmediately) {
      try { reserveAll(); } catch {}
    }

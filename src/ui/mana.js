...
// Удаление конкретной записи очереди (используем при управлении GSAP-потоком)
function removeReveal(ownerIndex, entry){
  if (!Number.isFinite(ownerIndex) || !entry) return;
  try {
    const root = getRevealQueueRoot();
    const queue = Array.isArray(root[ownerIndex]) ? root[ownerIndex] : null;
    ...
    setTimeout(() => {
      ...
      const triggerReveal = () => {
        ...
        if (visualOnly && Number.isFinite(ownerIndex)) {
          ...
          const revealInfo = (revealCount > 0)
            ? { from: prevVisible, to: visibleNow, amount: revealCount }
            : null;
          if (revealInfo) {
            enqueueReveal(ownerIndex, revealInfo);
          }
          const indices = [];
          for (let idx = prevVisible; idx < visibleNow; idx++) indices.push(idx);

          // Если сейчас работает GSAP-поток, убираем запись очереди заранее,
          // чтобы повторный пересбор панели после завершения анимации не схлопывал орбы.
          if (revealInfo && isRevealActive(ownerIndex)) {
            removeReveal(ownerIndex, revealInfo);
          }

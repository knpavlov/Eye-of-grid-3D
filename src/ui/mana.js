...
function syncBeforeManaSnapshot(ownerIndex) {
  ...
  try {
    ...
    if (!player) return;
    const pending = getPendingSlotSet(idx);
    const anonymous = getPendingAnonymous(idx);
    const pendingTotal = (pending?.size || 0) + Math.max(0, anonymous || 0);
    if (pendingTotal > 0) {
      const manaNow = Math.max(0, Number(player.mana) || 0);
      player._beforeMana = Math.max(0, manaNow - pendingTotal);
    } else if ('_beforeMana' in player) {
...
function resetPendingState(ownerIndex = null) {
  ...
  syncPendingState(state);
  if (ownerIndex == null) {
    syncBeforeManaSnapshot(0);
    syncBeforeManaSnapshot(1);
  } else {
    syncBeforeManaSnapshot(ownerIndex);
  }
  try { if (typeof window !== 'undefined' && typeof window.updateUI === 'function') window.updateUI(); } catch {}
}

...
function playSoulOrbRiseEffect(start, {
  durationMs = 500,
  riseHeight = 140,
  flightEase = 'power4.in',
  driftRange = 48,
} = {}) {
  ...
  const drift = (Math.random() - 0.5) * Math.max(20, driftRange);
  ...
  if (tl) {
    tl.to(orb, {
      ...
      .to(orb, {
        duration: Math.max(0.08, (duration / 1000) - 0.08),
        ease: flightEase || 'power4.in',
        opacity: 0,
        scale: 1.42,
        y: -rise,
        x: `+=${drift}`,
    ...
    orb.style.transitionTimingFunction = 'cubic-bezier(0.25, 0.1, 0.15, 1)';
    orb.style.transition = `transform ${duration}ms cubic-bezier(0.25, 0.1, 0.15, 1), opacity ${duration}ms cubic-bezier(0.25, 0.1, 0.15, 1)`;
    requestAnimationFrame(() => {
      orb.style.opacity = '1';
      orb.style.transform = `translate(-50%, -50%) translate(${drift}px, -${rise}px) scale(1.2)`;
...
export function animateManaGainFromWorld(pos, ownerIndex, visualOnly = true, targetSlotMaybe = null, optionsMaybe = {}) {
  try {
    ...
    const ownerIdx = Number.isFinite(ownerIndex) ? Math.max(0, Math.min(1, Math.floor(ownerIndex))) : null;
    const tasks = [];
    for (let i = 0; i < count; i += 1) {
      const slotIndex = (typeof targetSlot === 'number') ? (targetSlot + i) : null;
      const startMs = startDelayMs + delayBetweenMs * i;
      tasks.push({
        rawSlot: slotIndex,
        startMs,
        resolvedSlot: clampIndex(slotIndex),
      });
    }
    ...
    const gameState = hasWindow ? window.gameState : null;
    const useTimeline = hasWindow ? window.gsap?.timeline : null;
    const tl = useTimeline ? window.gsap.timeline({ paused: false }) : null;

    const resolveSlotForTask = (task) => {
      if (!task) return null;
      if (task.resolvedSlot != null) return task.resolvedSlot;

      const currentMana = Math.max(0, Number(gameState?.players?.[ownerIdx]?.mana) || 0);
      const barEl = hasWindow ? document.getElementById(`mana-display-${ownerIdx}`) : null;

      if (!visualOnly) {
        task.resolvedSlot = Math.max(0, Math.min(9, currentMana - 1));
        return task.resolvedSlot;
      }

      if (!barEl) {
        task.resolvedSlot = null;
        return task.resolvedSlot;
      }

      try {
        const children = Array.from(barEl.children || []);
        const filledNow = children.reduce((acc, el) => (
          acc + (el?.classList?.contains('mana-orb') ? 1 : 0)
        ), 0);
        const pendingSet = Number.isFinite(ownerIdx) ? getPendingSlotSet(ownerIdx) : new Set();
        const anonymous = Number.isFinite(ownerIdx) ? getPendingAnonymous(ownerIdx) : 0;
        const planned = filledNow + (pendingSet?.size || 0) + anonymous;
        task.resolvedSlot = Math.max(0, Math.min(9, planned));
      } catch (err) {
        console.warn('[mana] Не удалось определить слот панели маны, используем fallback:', err);
        task.resolvedSlot = Math.max(0, Math.min(9, currentMana));
      }
      return task.resolvedSlot;
    };

    const reserveForTask = (task) => {
      if (!visualOnly || ownerIdx == null) return;
      try {
        const slot = resolveSlotForTask(task);
        reserveIncomingOrbs(ownerIdx, slot != null ? [slot] : [], 1);
      } catch (err) {
        console.error('[mana] Не удалось зарезервировать ячейку маны:', err);
      }
    };

    const launchForTask = (task) => {
      try {
        ...
        const slotForReveal = resolveSlotForTask(task);

        if (visualOnly && ownerIdx != null && !reserveImmediately) {
          reserveIncomingOrbs(ownerIdx, slotForReveal != null ? [slotForReveal] : [], 1);
        }
        ...
        if (visualOnly && ownerIdx != null) {
          const slotToRelease = resolveSlotForTask(task);
          releaseIncomingOrb(ownerIdx, slotToRelease);
        }
      }
    };

    const revealForTask = (task) => {
      try {
        const slotForReveal = resolveSlotForTask(task);
        playPanelOrbReveal(ownerIdx, slotForReveal, { visualOnly });
      } catch (err) {
    ...
    for (const task of tasks) {
      const reserveLead = reserveImmediately ? Math.max(0, task.startMs - 90) : null;
      if (tl) {
        if (reserveLead != null) {
          tl.call(() => reserveForTask(task), null, reserveLead / 1000);
        }
        tl.call(() => launchForTask(task), null, task.startMs / 1000);
        tl.call(() => revealForTask(task), null, (task.startMs + flightDurationMs) / 1000);
      } else {
        if (reserveLead != null) setTimeout(() => reserveForTask(task), reserveLead);
        setTimeout(() => launchForTask(task), task.startMs);
        setTimeout(() => revealForTask(task), task.startMs + flightDurationMs);
      }
    }

    if (visualOnly && ownerIdx != null) {
      const finishMs = tasks[tasks.length - 1].startMs + flightDurationMs + 140;
      setTimeout(() => {
        try {
          const player = window.gameState?.players?.[ownerIdx];
          if (player && player._beforeMana != null) {

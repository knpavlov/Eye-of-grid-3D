...
function playSoulOrbRiseEffect(start, { durationMs = 500, riseHeight = 140 } = {}) {
  ...
  if (tl) {
    tl.to(orb, {
      ...
      .to(orb, {
        duration: Math.max(0.06, (duration / 1000) - 0.04),
        ease: 'power3.in',
        opacity: 0,
        scale: 1.35,
        y: -rise,
    ...
    const scheduleSingle = (slotIndex, extraDelayMs) => {
      ...
      const reserveSlot = () => {
        ...
        try {
          ...
          reserveIncomingOrbs(ownerIndex, slotToReserve != null ? [slotToReserve] : [], 1);
          reserved = true;
        } catch {}
      };

      if (reserveImmediately && visualOnly && Number.isFinite(ownerIndex)) {
        reserveSlot();
      }

      const runEffect = () => {
        try {
          if (visualOnly && Number.isFinite(ownerIndex) && !reserved) {
            reserveSlot();
          }
          ...
          if (visualOnly && Number.isFinite(ownerIndex) && !reserveImmediately) {
            try {
              reserveIncomingOrbs(ownerIndex, slotForReveal != null ? [slotForReveal] : [], 1);
            } catch {}
          }

...
export function renderBars(gameState) {
  ...
  for (let p = 0; p < 2; p++) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      // Сброс трансформации по умолчанию, чтобы орбы не «зависали» уменьшенными
      ...
      try {
        ...
        if (tl) {
          ...
          tl.to(container, { opacity: 1, scale: 1, duration: 0.22, ease: 'back.out(1.8)' })
            ...
            .add('reveal', Math.max(0.22, floatSeconds - 0.28))
          ...
          setTimeout(() => { triggerReveal(); }, Math.max(0, floatMs * 0.6));

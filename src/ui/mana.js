...
function normalizeOrbStyles(bar, indices) {
  if (!bar) return;
  const applyTo = (idx) => {
  ...
  if (Array.isArray(indices) && indices.length) {
    for (const idx of indices) applyTo(idx);
  } else {
    const total = bar.children?.length || 0;
    for (let i = 0; i < total; i++) applyTo(i);
  }
}
            ...
            try {
              ...
              if (bar && typeof window !== 'undefined' && window.gsap?.timeline) {
                ...
                const finalizeReveal = () => {
                  ...
                  normalizeOrbStyles(bar, indices);

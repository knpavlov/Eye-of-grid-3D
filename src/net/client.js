...
(() => {
  ...
  function digest(state){
    try {
      ...
      const compact = {
        ...
        pendingDiscards: (state.pendingDiscards||[]).map(req => req ? ({ id: req.id, target: req.target, remaining: req.remaining, requested: req.requested, autoEnd: !!req.autoEndTurn }) : null)
      };
  ...
  function updateInputLock(){
    ...
    if (shouldLock) {
      try {
        if (typeof window !== 'undefined') {
          const discardMgr = window.__ui?.discardManager;
          if (discardMgr?.hasActiveLocalRequest?.() || discardMgr?.isOverlayActive?.()) {
            shouldLock = false;
          }
        }
      } catch {}

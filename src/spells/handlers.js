...
function playMagicDamageFx(r, c) {
  const tile = getTileMeshAt(r, c);
  if (!tile || !tile.position || typeof tile.position.clone !== 'function') return;
  try {
    const pos = tile.position.clone();
    pos.y += 1.2;
    if (typeof window !== 'undefined' && window.__fx?.magicBurst) {
...
function updateDominionPrompt(pending) {
  ...
  let text = `Сбросьте существо из руки (осталось ${remaining}).`;
...
function handleDominionDiscard(handIdx) {
  ...
  const staged = Array.isArray(pending.stagedDiscards) ? pending.stagedDiscards : (pending.stagedDiscards = []);
  staged.push({ tpl: chosenTpl, handIndex: handIdx });
...
function finalizeDominionSpell() {
  ...
  if (damage > 0 && targets.length) {
    for (const target of targets) {
      ...
      playMagicDamageFx(target.r, target.c);
  ...
  if (Array.isArray(pending.stagedDiscards)) {
    pending.stagedDiscards.length = 0;
  }
...
function cancelElementalDominionSelection({ silent = false } = {}) {
  const pending = interactionState.pendingSpellElementalDominion;

  if (pending?.player && Array.isArray(pending.stagedDiscards) && pending.stagedDiscards.length) {
    const player = pending.player;
    const graveyard = Array.isArray(player.graveyard) ? player.graveyard : (player.graveyard = []);
    const staged = pending.stagedDiscards.slice().reverse();
    for (const entry of staged) {
      if (!entry?.tpl) continue;
      const gyIndex = graveyard.lastIndexOf(entry.tpl);
      if (gyIndex >= 0) {
        graveyard.splice(gyIndex, 1);
      }
      const insertAt = Number.isInteger(entry.handIndex)
        ? Math.max(0, Math.min(entry.handIndex, player.hand.length))
        : player.hand.length;
      player.hand.splice(insertAt, 0, entry.tpl);
    }
    pending.stagedDiscards.length = 0;
    pending.discards = [];
  ...
  try { window.__ui?.panels?.hidePrompt?.(); } catch {}
  try { window.__ui?.cancelButton?.refreshCancelButton?.(); } catch {}
  if (!silent && pending?.tpl) {
    addLog(`${pending.tpl.name}: розыгрыш отменён.`);
  }
}
...
function forceSyncState(reason = 'riotous-impunity') {
  if (typeof window === 'undefined') return;
  const schedule = window.schedulePush;
  if (typeof schedule === 'function') {
    try { schedule(reason, { force: true }); } catch {}
  }
}

...
function finalizeMesmerLapseDiscard(handIdx) {
  ...
  try {
    ...
    const idx = Array.isArray(gy)
      ? gy.map(card => card && card.id).lastIndexOf(chosenTpl.id)
      : -1;
    if (idx >= 0) gy.splice(idx, 1);
  } catch {}
  try {
    player.offeredToEye = Array.isArray(player.offeredToEye) ? player.offeredToEye : [];
    player.offeredToEye.push(chosenTpl);
  } catch {}
  ...
  addLog(`${spellTpl.name}: ${creatureName} принесён(а) Оку.`);
  ...
  SPELL_SUMMONER_MESMERS_LAPSE: {
    onCast({ tpl, pl, idx, cardMesh, tileMesh }) {
      ...
      if (!unitIndices.length) {
        showNotification('В руке нет существ для предложения Оку', 'error');
        if (cardMesh) returnCardToHand(cardMesh);
        return;
      }
      ...
      try {
        window.__ui.panels.showPrompt(
          'Выберите существо в руке для предложения Оку',
          () => {
            cancelMesmerLapseSelection();
            updateUI();
          },

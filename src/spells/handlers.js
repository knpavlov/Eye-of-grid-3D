...
import { enqueueDrainOnlyManaEvent } from '../core/abilityHandlers/manaSteal.js';
import { rememberManaStealEvent } from '../ui/manaStealRegistry.js';
...
function finalizeMesmerLapseDiscard(handIdx) {
  ...
  if (opponent) {
    ...
    if (actualLoss > 0) {
      ...
      let drainEvent = null;
      try {
        drainEvent = enqueueDrainOnlyManaEvent(state, {
          from: opponentIndex,
          amount: actualLoss,
          before,
          after,
        ...
        console.warn('[spell] Не удалось зарегистрировать событие потери маны:', err);
      }
      if (!drainEvent) {
        drainEvent = {
          amount: actualLoss,
      ...
      try {
        rememberManaStealEvent(drainEvent);
      } catch {}
      try {
        ...
        console.warn('[spell] Не удалось запустить анимацию потери маны:', err);
      }
      try {
        if (typeof window !== 'undefined') {
          const mySeat = (typeof window.MY_SEAT === 'number') ? window.MY_SEAT : null;
          if (mySeat == null || mySeat === opponentIndex) {
            ...
            if (typeof notifyDrain === 'function') {
              notifyDrain(opponentIndex, actualLoss, { source: 'MESMER_LAPSE', immediate: true });
            }
          }
        }
      } catch (err) {
        console.warn('[spell] Не удалось показать уведомление о потере маны:', err);
      }
    } else if (manaLoss > 0) {

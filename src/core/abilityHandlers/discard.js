...
export function applyDeathDiscardEffects(state, deaths = [], context = {}) {
  ...
  const shouldAutoEndTurn = !!context?.autoEndTurn;
  ...
  for (const death of deaths) {
    ...
    if (cfgOn?.element && fieldElement === cfgOn.element) {
      ...
      const req = makeRequest(state, {
        ...
        count,
        source: baseSource,
        durationMs: cfgOn.durationMs || DEFAULT_TIMER_MS,
        autoEndTurn: shouldAutoEndTurn,
      });
      if (req?.skipped) {
    ...
    if (cfgOff?.element && fieldElement && fieldElement !== cfgOff.element) {
      ...
      const req = makeRequest(state, {
        ...
        count,
        source: baseSource,
        durationMs: cfgOff.durationMs || DEFAULT_TIMER_MS,
        autoEndTurn: shouldAutoEndTurn,
      });
      if (req?.skipped) {
        events.logs.push(`${baseSource.name}: оппоненту нечего сбрасывать.`);

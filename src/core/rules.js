...
import { createDeathEntry } from './abilityHandlers/deathSummary.js';
...
export function stagedAttack(state, r, c, opts = {}) {
  ...
  function finish() {
    ...
    if (u && (u.currentHP ?? CARDS[u.tplId].hp) <= 0) {
      const deathEntry = createDeathEntry(nFinal, u, rr, cc)
        || { r: rr, c: cc, owner: u.owner, tplId: u.tplId, uid: u.uid ?? null, element: cellRef?.element || null };
  ...
  try {
    ...
    const repositionInfo = applyDeathRepositionEffects(nFinal, deaths);
    if (Array.isArray(repositionInfo?.logs) && repositionInfo.logs.length) {
      logLines.push(...repositionInfo.logs);
    }
...
export function magicAttack(state, fr, fc, tr, tc) {
  ...
  const repositionInfo = applyDeathRepositionEffects(n1, deaths);
  if (Array.isArray(repositionInfo?.logs) && repositionInfo.logs.length) {
    logLines.push(...repositionInfo.logs);
  }

...
import { hasQuickness, hasPerfectDodge } from './abilities.js';
...
export function stagedAttack(state, r, c, opts = {}) {
  ...
  const attackerQuick = hasQuickness(tplA);
  const quickSources = [];
  ...
  for (const h of hitsRaw) {
    ...
    if (hasQuickness(tplB) && !attackerQuick) {
      const hitsB = computeHits(base, h.r, h.c, { target: { r, c }, union: true });
      if (hitsB.length) {
        const { atk: batk } = effectiveStats(base.board[h.r][h.c], B);
        const isMagicB = CARDS[B.tplId].attackType === 'MAGIC';
        const attackerCellEl = base.board[r][c]?.element;
        const dodgeA = hasPerfectDodge(tplA, attackerCellEl) && !isMagicB;
        if (!dodgeA) {
          quickRetaliation += Math.max(0, batk);
  ...
  const step1Damages = hitsRaw.map(h => {
    ...
    const perfect = hasPerfectDodge(CARDS[B.tplId], cell?.element) && !isMagic;
    const dodge50 = !perfect && CARDS[B.tplId].dodge50 && !isMagic && Math.random() < 0.5;
    const dealt = (perfect || dodge50) ? 0 : h.dmg;
    return { ...h, dealt, dodge: perfect || dodge50 };
  });
  ...
  function doStep1() {
    ...
    if (quickRetaliation > 0) {
      const before = attacker.currentHP ?? attacker.hp;
      const afterHP = Math.max(0, before - quickRetaliation);
      attacker.currentHP = afterHP;
      const aN1 = n1.board?.[r]?.[c]?.unit;
      if (aN1) aN1.currentHP = afterHP;
      logLines.push(`${quickSources.join(', ')} → ${CARDS[attacker.tplId].name}: ${quickRetaliation} dmg (HP ${before}→${afterHP})`);
    }
    if ((attacker.currentHP ?? attacker.hp) > 0) {
  ...
  function step2() {
    ensureStep1();
    if (quickRetaliation > 0 && !attackerQuick) {
      return { total: 0, retaliators: [] };
    }
    ...
    for (const h of step1Damages) {
      ...
      if (hitsB.length) {
        ...
        const isMagicB = CARDS[B.tplId].attackType === 'MAGIC';
        const attackerCellEl = n1.board[r][c]?.element;
        const dodgeA = hasPerfectDodge(tplA, attackerCellEl) && !isMagicB;
        if (!dodgeA) {

...
export function stagedAttack(state, r, c, opts = {}) {
  ...
  for (const h of hitsRaw) {
    ...
    // Быстрота у обоих сторон взаимно нейтрализуется: контратака до удара
    // случается только если защитник быстрый, а атакующий — нет
  ...
  function step2() {
    ...
    for (const h of step1Damages) {
      ...
      if (!alive) continue;
      // если защитник быстрый и уже успел контратаковать первым,
      // второй раз он не отвечает

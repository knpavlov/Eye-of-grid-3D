...
// вычисление базовой стоимости активации без учёта скидок на атаку
...
// Стоимость атаки с учётом возможных скидок
export function attackCost(tpl, fieldElement) {
  let cost = activationCost(tpl);
  if (tpl && tpl.attackCostReduction) {
    cost = Math.max(0, cost - tpl.attackCostReduction);
  }
  const onElem = tpl && tpl.attackCostReductionOnElement;
...
// Проверка наличия способности "идеальное уклонение"
export const hasPerfectDodge = (tpl) => !!(tpl && tpl.perfectDodge);

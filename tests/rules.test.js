...
describe('особые способности', () => {
  ...
  it('повторное получение dodge от двух старейшин учитывает уже израсходованные попытки', () => {
    const state = { board: makeBoard(), players:[{mana:0},{mana:0}], turn:1 };
    state.board[1][1].unit = { owner:0, tplId:'WATER_MONK_ELDER_OF_OKUNADA', facing:'N' };
    state.board[1][2].unit = { owner:0, tplId:'EARTH_NOVOGUS_CATAPULT', facing:'W', currentHP: CARDS.EARTH_NOVOGUS_CATAPULT.hp };

    refreshBoardDodgeStates(state);

    const catapult = state.board[1][2].unit;
    expect(catapult.dodgeState).toBeTruthy();
    expect(catapult.dodgeState?.remaining ?? 0).toBe(1);

    const firstUse = attemptUnitDodge(state, 1, 2, { rng: () => 0.1 });
    expect(firstUse).toMatchObject({ success: true, consumed: true });
    expect(catapult.dodgeState?.remaining ?? 0).toBe(0);
    expect(catapult.dodgeState?.successesUsed ?? 0).toBe(1);

    state.board[0][2].unit = { owner:0, tplId:'WATER_MONK_ELDER_OF_OKUNADA', facing:'S' };
    refreshBoardDodgeStates(state);

    expect(catapult.dodgeState).toBeTruthy();
    expect(catapult.dodgeState?.max ?? 0).toBeGreaterThanOrEqual(2);
    expect(catapult.dodgeState?.remaining ?? 0).toBe(1);
    expect(catapult.dodgeState?.successesUsed ?? 0).toBe(1);

    const secondUse = attemptUnitDodge(state, 1, 2, { rng: () => 0.1 });
    expect(secondUse).toMatchObject({ success: true, consumed: true });
    expect(catapult.dodgeState?.remaining ?? 0).toBe(0);
    expect(catapult.dodgeState?.successesUsed ?? 0).toBeGreaterThanOrEqual(2);
  });

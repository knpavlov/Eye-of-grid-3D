...
import { computeCellBuff, effectiveStats, hasAdjacentGuard, computeHits, stagedAttack, magicAttack } from '../src/core/rules.js';
...
describe('guards and hits', () => {
  ...
  it('computeHits: Great Minos бьёт только на дистанции 2', () => {
    const state = { board: makeBoard() };
  ...
  it('perfect dodge защищает от обычных атак', () => {
    ...
    state.board[1][1].unit = { owner: 0, tplId: 'FIRE_PARTMOLE_FLAME_LIZARD', facing: 'N', hp: 2 };
    state.board[0][1].unit = { owner: 1, tplId: 'FIRE_GREAT_MINOS', facing: 'S', hp: 1 };
    const atk = stagedAttack(state, 1, 1);
    const res = atk.finish();
    expect(res.n1.board[0][1].unit.currentHP).toBe(1);
  });

  it('quickness позволяет защитнику ударить первым', () => {
    CARDS.TEST_QUICK_DEF = { id: 'TEST_QUICK_DEF', name: 'Quick Defender', type: 'UNIT', cost: 0, element: 'FIRE', atk: 2, hp: 1, attackType: 'STANDARD', firstStrike: true, attacks: [ { dir: 'N', ranges: [1] } ] };
    CARDS.TEST_WEAK = { id: 'TEST_WEAK', name: 'Weakling', type: 'UNIT', cost: 0, element: 'FIRE', atk: 1, hp: 1, attackType: 'STANDARD', attacks: [ { dir: 'N', ranges: [1] } ] };
    const state = { board: makeBoard(), players: [{ mana: 0 }, { mana: 0 }], turn: 1 };
    state.board[1][1].unit = { owner: 0, tplId: 'TEST_WEAK', facing: 'N', hp: 1 };
    state.board[0][1].unit = { owner: 1, tplId: 'TEST_QUICK_DEF', facing: 'S', hp: 1 };
    const atk = stagedAttack(state, 1, 1);
    const res = atk.finish();
    expect(res.n1.board[1][1].unit).toBeNull();
    delete CARDS.TEST_QUICK_DEF; delete CARDS.TEST_WEAK;
  });
});

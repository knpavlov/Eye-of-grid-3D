...
describe('guards and hits', () => {
  ...
  it('quickness retaliates before attack', () => {
    const state = { board: makeBoard(), players: [{ mana: 10 }, { mana: 10 }], turn: 1 };
    state.board[1][1].unit = { owner: 1, tplId: 'FIRE_PARTMOLE_FLAME_LIZARD', facing: 'N', hp: 2 };
    state.board[0][1].unit = { owner: 0, tplId: 'FIRE_FLAME_MAGUS', facing: 'S', hp: 1 };
    const res = stagedAttack(state, 0, 1);
    const after = res.finish();
    expect(after.n1.board[0][1].unit).toBeNull();
    const defender = after.n1.board[1][1].unit;
    expect(defender.currentHP ?? defender.hp).toBe(2);
  });

  it('perfect dodge prevents standard damage', () => {
    const state = { board: makeBoard(), players: [{ mana: 10 }, { mana: 10 }], turn: 1 };
    state.board[1][1].unit = { owner: 0, tplId: 'FIRE_GREAT_MINOS', facing: 'N', hp: 1 };
    state.board[0][1].unit = { owner: 1, tplId: 'FIRE_PARTMOLE_FLAME_LIZARD', facing: 'S', hp: 2 };
    const res = stagedAttack(state, 0, 1);
    const after = res.finish();
    const hp = after.n1.board[1][1].unit.currentHP ?? after.n1.board[1][1].unit.hp;
    expect(hp).toBe(1);
  });

  it('perfect dodge does not prevent magic damage', () => {
    const state = { board: makeBoard(), players: [{ mana: 10 }, { mana: 10 }], turn: 1 };
    state.board[1][1].unit = { owner: 1, tplId: 'FIRE_GREAT_MINOS', facing: 'N', hp: 1 };
    state.board[0][1].unit = { owner: 0, tplId: 'FIRE_FLAME_MAGUS', facing: 'S', hp: 1 };

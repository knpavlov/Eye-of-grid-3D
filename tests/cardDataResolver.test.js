...
describe('resolveCardReference', () => {
  ...
  it('находит шаблон по названию, если id отсутствует', () => {
    const { cardData, artRef } = resolveCardReference({ name: 'Flame Magus' }, { cardsDb: SAMPLE_DB });
    expect(cardData?.id).toBe('FIRE_FLAME_MAGUS');
    expect(artRef?.id).toBe('FIRE_FLAME_MAGUS');
  });
});
...
describe('resolveUnitCardInfo', () => {
  ...
  it('подбирает шаблон по имени юнита, когда передан только name', () => {
    const unit = { owner: 1, name: 'Swordfish' };
    const { cardData, artRef } = resolveUnitCardInfo(unit, { cardsDb: SAMPLE_DB });
    expect(cardData?.id).toBe('WATER_SWORDFISH');
    expect(cardData?.name).toBe('Swordfish');
    expect(artRef?.id).toBe('WATER_SWORDFISH');
  });
  ...
  it('подбирает шаблон по tplId в нижнем регистре', () => {
    const unit = { owner: 1, tplId: 'fire_flame_magus' };
    const { cardData, artRef } = resolveUnitCardInfo(unit, { cardsDb: SAMPLE_DB });
    expect(cardData?.id).toBe('FIRE_FLAME_MAGUS');
    expect(artRef?.id).toBe('FIRE_FLAME_MAGUS');
  });

  it('подбирает шаблон по tplId с дефисами', () => {
    const unit = { owner: 0, tplId: 'biolith-bomber' };
    const { cardData, artRef } = resolveUnitCardInfo(unit, { cardsDb: SAMPLE_DB });
    expect(cardData?.id).toBe('BIOLITH_BOMBER');
    expect(artRef?.id).toBe('BIOLITH_BOMBER');
  });
});
...
describe('normalizeArtRef', () => {
  ...
  it('канонизирует смешанный идентификатор', () => {
    expect(normalizeArtRef(' biolith-bomber ')).toEqual({ id: 'BIOLITH_BOMBER' });
  });
});

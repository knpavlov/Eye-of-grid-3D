...
describe('resolveUnitCardInfo', () => {
  ...
  it('сохраняет шаблонный id и выносит уникальный идентификатор экземпляра', () => {
    const unit = {
      owner: 0,
      id: 'UNIT-42',
      tplId: 'FIRE_FLAME_MAGUS',
      card: { id: 'UNIT-42', tplId: 'FIRE_FLAME_MAGUS', atk: 3 },
    };
    const { cardData, artRef } = resolveUnitCardInfo(unit, { cardsDb: SAMPLE_DB });
    expect(cardData?.id).toBe('FIRE_FLAME_MAGUS');
    expect(cardData?.instanceId).toBe('UNIT-42');
    expect(artRef?.id).toBe('FIRE_FLAME_MAGUS');
  });

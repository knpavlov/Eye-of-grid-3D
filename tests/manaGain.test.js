...
describe('прирост маны при гибели существ', () => {
  ...
  it('план анимаций объединяет базовую и бонусную ману в одно событие', () => {
    ...
    expect(plan.events[0].slots).toEqual([3, 4, 5]);
    expect(typeof plan.schedule).toBe('function');
  });

  it('события одного владельца идут с шагом 320 мс и следуют за соответствующей смертью', () => {
    const makeVec = (x = 0, y = 0, z = 0) => ({
      x, y, z,
      clone() { return makeVec(this.x, this.y, this.z); },
      add(v = {}) {
    ...
    expect(plan.events[0].startDelayMs).toBe(240);
    expect(plan.events[1].startDelayMs).toBe(560);
    expect(plan.events[0].slots).toEqual([0, 1, 2]);
    expect(plan.events[1].slots).toEqual([3, 4]);
  });
});

...
describe('прирост маны при гибели существ', () => {
  ...
  it('план анимаций строит отдельные события для базовой и бонусной маны', () => {
    ...
    expect(plan.events[0].eventDurationMs).toBeGreaterThanOrEqual(700);
    expect(plan.events[1].eventDurationMs).toBeGreaterThanOrEqual(700);
    expect(plan.events[0].slots).toEqual([3]);
    expect(plan.events[1].slots).toEqual([4, 5]);
    expect(typeof plan.schedule).toBe('function');
  });

  it('события одного владельца идут с шагом 300 мс и следуют за соответствующей смертью', () => {
    const makeVec = (x = 0, y = 0, z = 0) => ({
      x, y, z,
      clone() { return makeVec(this.x, this.y, this.z); },
      add(v = {}) {
    ...
    const tileMeshes = [
      [null, null, null],
      [null, { position: makeVec() }, { position: makeVec(1, 0, 0) }],
      [null, null, null],
    ];

    const plan = buildManaGainPlan({
      playersBefore: [{ mana: 0 }, { mana: 0 }],
      deaths: [
        { owner: 0, r: 1, c: 1, tplId: 'EARTH_SKELETON_SOLDIER' },
        { owner: 0, r: 1, c: 2, tplId: 'EARTH_NOVOGUS_CATAPULT' },
      ],
      manaGainEntries: [
        { owner: 0, amount: 2, death: { owner: 0, r: 1, c: 1 } },
        { owner: 0, amount: 1, death: { owner: 0, r: 1, c: 2 } },
      ],
      tileMeshes,
      THREE: null,
    });
    ...
    const ordered = plan.events
      .slice()
      .sort((a, b) => a.startDelayMs - b.startDelayMs);

    expect(ordered.map(ev => ev.amount)).toEqual([1, 2, 1, 1]);
    expect(ordered[0].slots).toEqual([0]);
    expect(ordered[1].slots).toEqual([1, 2]);
    expect(ordered[2].slots).toEqual([3]);
    expect(ordered[3].slots).toEqual([4]);

    for (let i = 1; i < ordered.length; i += 1) {
      expect(ordered[i].startDelayMs - ordered[i - 1].startDelayMs).toBeGreaterThanOrEqual(700);
    }
  });
});

...
describe('прирост маны при гибели существ', () => {
  ...
  it('план анимаций строит отдельные события для базовой и бонусной маны', () => {
    ...
    expect(plan.events[0].startDelayMs).toBeGreaterThanOrEqual(0);
    expect(plan.events[1].startDelayMs)
      .toBe(plan.events[0].startDelayMs + plan.events[0].delayBetweenMs * plan.events[0].amount);
    expect(plan.events[1].delayBetweenMs).toBe(plan.events[0].delayBetweenMs);

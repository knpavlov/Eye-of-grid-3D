...
describe('прирост маны при гибели существ', () => {
  ...
  it('план анимаций объединяет базовую и бонусную ману в один запуск с задержкой 0.3с', () => {
    ...
    expect(plan.events[0].amount).toBe(3);
    expect(plan.events[0].delayBetweenMs).toBe(300);
    expect(plan.events[0].startDelayMs).toBeGreaterThanOrEqual(360);
    expect(typeof plan.schedule).toBe('function');
  });

  it('событие наблюдателя стартует после орбов смерти и использует позицию наблюдателя', () => {
    const makeVec = (x = 0, y = 0, z = 0) => ({
      x, y, z,
      clone() { return makeVec(this.x, this.y, this.z); },
      add(v = {}) {
    ...
    const tileMeshes = [
      [{ position: makeVec(0, 0, 0) }, null, null],
      [null, { position: makeVec(1, 1, 1) }, null],
      [null, null, null],
    ];

    const plan = buildManaGainPlan({
      playersBefore: [{ mana: 1 }, { mana: 0 }],
      deaths: [{ owner: 0, r: 1, c: 1, tplId: 'EARTH_SKELETON_SOLDIER' }],
      manaGainEntries: [{ owner: 0, amount: 1, death: { owner: 0, r: 1, c: 1 }, watcher: { r: 0, c: 0 } }],
      tileMeshes,
      THREE: null,
    });

    expect(plan.events.length).toBe(2);
    const deathEvent = plan.events.find(ev => ev.slotStart === 1);
    const watcherEvent = plan.events.find(ev => Math.abs(ev.origin.x) < 0.001 && Math.abs(ev.origin.y - 1.2) < 0.001);
    expect(deathEvent).toBeTruthy();
    expect(watcherEvent).toBeTruthy();
    expect(watcherEvent.startDelayMs).toBeGreaterThanOrEqual(deathEvent.startDelayMs + 300);
  });
});

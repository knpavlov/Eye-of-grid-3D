...
describe('прирост маны при гибели существ', () => {
  ...
  it('события одного владельца идут с шагом 300 мс и следуют за соответствующей смертью', () => {
    ...
    for (const ev of plan.events) {
      for (let i = 0; i < ev.amount; i += 1) {
        starts.push(ev.startDelayMs + ev.delayBetweenMs * i);
      }
    }
    const ordered = starts.slice().sort((a, b) => a - b);
    for (let i = 1; i < ordered.length; i += 1) {
      expect(ordered[i] - ordered[i - 1]).toBe(300);
    }
    expect(plan.events[0].slots[0]).toBe(0);
    expect(plan.events[1].slots[0]).toBe(1);
    expect(plan.events[2].slots[0]).toBe(3);
  });
});

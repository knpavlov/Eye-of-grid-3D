...
describe('прирост маны при гибели существ', () => {
  ...
  it('орбы одной смерти вылетают последовательно с шагом 0.3 секунды', () => {
    const makeVec = (x = 0, y = 0, z = 0) => ({
      x, y, z,
      clone() { return makeVec(this.x, this.y, this.z); },
      add(v = {}) {
    ...
    expect(plan.events.length).toBe(2);
    const baseEvent = plan.events.find(ev => ev.amount === 1);
    const bonusEvent = plan.events.find(ev => ev.amount === 2);
    expect(baseEvent).toBeTruthy();
    expect(bonusEvent).toBeTruthy();
    if (!baseEvent || !bonusEvent) return;
    expect(baseEvent.delayBetweenMs).toBe(300);
    expect(bonusEvent.delayBetweenMs).toBe(300);
    expect(bonusEvent.startDelayMs - baseEvent.startDelayMs).toBe(300);
  });
});

...
describe('fieldquake-эффекты биолит-карт', () => {
  ...
  it('Novogus наносит урон до fieldquake, цель не возрождается за счёт смены поля', () => {
    const state = {
      board: makeBoard('WATER'),
      players: [
        { mana: 10 },
        { mana: 10 },
      ],
    ...
    const result = magicAttack(state, 1, 1, 0, 1);

    expect(result).not.toBeNull();
    expect(result.fieldquakes).toHaveLength(1);
    expect(result.fieldquakes[0]).toMatchObject({ prevElement: 'WATER', nextElement: 'FIRE' });
    expect(result.n1.board[0][1].unit).toBeNull();
  });

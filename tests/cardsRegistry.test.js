...
describe('mergeExternalCardTemplates', () => {
  ...
  it('находит каноническую карту по совпадающему имени даже при неизвестном id', () => {
    const randomId = 'srv::opponent::flame_magus';
    expect(CARDS[randomId]).toBeUndefined();
    mergeExternalCardTemplates([{ id: randomId, name: 'Flame Magus', type: 'UNIT', element: 'FIRE', atk: 1, hp: 1, cost: 1 }]);
    expect(CARDS[randomId]).toBe(CARDS.FIRE_FLAME_MAGUS);
    expect(CARDS.FIRE_FLAME_MAGUS).toBeTruthy();
    delete CARDS[randomId];
  });

  it('подбирает существующую карту по номеру и параметрам из внешнего шаблона', () => {
    const randomId = 'srv::lizard_alias';
    const before = CARDS.FIRE_PARTMOLE_FLAME_LIZARD;
    expect(before).toBeTruthy();
    mergeExternalCardTemplates([{ id: randomId, cardNumber: before.cardNumber, element: before.element, cost: before.cost, atk: before.atk, hp: before.hp }]);
    expect(CARDS[randomId]).toBe(before);
    delete CARDS[randomId];
  });
});
